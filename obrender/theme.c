...
static gboolean read_color(XrmDatabase db, const RrInstance* inst, const gchar* rname, RrColor** value);
static gboolean read_mask(const RrInstance* inst, const gchar* path, const gchar* maskname, RrPixmapMask** value);
static gboolean read_appearance(XrmDatabase db,
...
static void read_button_styles(XrmDatabase db,
                               const RrInstance* inst,
                               gchar* path,
                               const RrTheme* theme,
                               RrButton* btn,
...
#define READ_INT(x_resstr, x_var, x_min, x_max, x_def)                   \
  if (!read_int(db, x_resstr, &x_var) || x_var < x_min || x_var > x_max) \
    x_var = x_def;
...
#define READ_COLOR_(x_res1, x_res2, x_var, x_def)                                     \
  if (!read_color(db, inst, x_res1, &x_var) && !read_color(db, inst, x_res2, &x_var)) \
    x_var = x_def;
...
#define READ_APPEARANCE_(x_res1, x_res2, x_var, x_parrel, x_defval)                                                 \
  if (!read_appearance(db, inst, x_res1, x_var, x_parrel) && !read_appearance(db, inst, x_res2, x_var, x_parrel)) { \
    RrAppearanceFree(x_var);                                                                                        \
    x_var = RrAppearanceCopy(x_defval);                                                                             \
  }

RrTheme* RrThemeNew(const RrInstance* inst,
                    const gchar* name,
  ...
  READ_COLOR("window.undecorated.active.border.color", theme->frame_undecorated_focused_border_color,
             RrColorCopy(theme->frame_focused_border_color));
  READ_COLOR("window.active.title.separator.color", theme->title_separator_focused_color,
             RrColorCopy(theme->frame_focused_border_color));

  READ_COLOR("window.inactive.border.color", theme->frame_unfocused_border_color,
             RrColorCopy(theme->frame_focused_border_color));

  READ_COLOR("window.undecorated.inactive.border.color", theme->frame_undecorated_unfocused_border_color,
  ...
  READ_COLOR("menu.border.color", theme->menu_border_color, RrColorCopy(theme->frame_focused_border_color));

  READ_COLOR("osd.border.color", theme->osd_border_color, RrColorCopy(theme->frame_focused_border_color));

  READ_COLOR("window.active.client.color", theme->cb_focused_color, RrColorNew(inst, 0xff, 0xff, 0xff));

  READ_COLOR("window.inactive.client.color", theme->cb_unfocused_color, RrColorNew(inst, 0xff, 0xff, 0xff));

  READ_COLOR("window.active.label.text.color", theme->title_focused_color, RrColorNew(inst, 0x0, 0x0, 0x0));

  READ_COLOR("window.inactive.label.text.color", theme->title_unfocused_color, RrColorNew(inst, 0xff, 0xff, 0xff));

  READ_COLOR_("osd.active.label.text.color", "osd.label.text.color", theme->osd_text_active_color,
              RrColorCopy(theme->title_focused_color));

  READ_COLOR_("osd.inactive.label.text.color", "osd.label.text.color", theme->osd_text_inactive_color,
              RrColorCopy(theme->title_unfocused_color));

  READ_COLOR("window.active.button.unpressed.image.color", theme->titlebut_focused_unpressed_color,
             RrColorNew(inst, 0, 0, 0));

  READ_COLOR("window.inactive.button.unpressed.image.color", theme->titlebut_unfocused_unpressed_color,
             RrColorNew(inst, 0xff, 0xff, 0xff));

  READ_COLOR("window.active.button.pressed.image.color", theme->titlebut_focused_pressed_color,
  ...
  READ_COLOR("window.active.button.disabled.image.color", theme->titlebut_focused_disabled_color,
             RrColorNew(inst, 0xff, 0xff, 0xff));

  READ_COLOR("window.inactive.button.disabled.image.color", theme->titlebut_unfocused_disabled_color,
             RrColorNew(inst, 0, 0, 0));

  READ_COLOR("window.active.button.hover.image.color", theme->titlebut_focused_hover_color,
             RrColorCopy(theme->titlebut_focused_unpressed_color));

  READ_COLOR("window.inactive.button.hover.image.color", theme->titlebut_unfocused_hover_color,
             RrColorCopy(theme->titlebut_unfocused_unpressed_color));

  READ_COLOR_("window.active.button.toggled.unpressed.image.color", "window.active.button.toggled.image.color",
              theme->titlebut_focused_unpressed_toggled_color, RrColorCopy(theme->titlebut_focused_pressed_color));

  READ_COLOR_("window.inactive.button.toggled.unpressed.image.color", "window.inactive.button.toggled.image.color",
              theme->titlebut_unfocused_unpressed_toggled_color, RrColorCopy(theme->titlebut_unfocused_pressed_color));

  READ_COLOR("window.active.button.toggled.hover.image.color", theme->titlebut_focused_hover_toggled_color,
             RrColorCopy(theme->titlebut_focused_unpressed_toggled_color));

  READ_COLOR("window.inactive.button.toggled.hover.image.color", theme->titlebut_unfocused_hover_toggled_color,
             RrColorCopy(theme->titlebut_unfocused_unpressed_toggled_color));

  READ_COLOR("window.active.button.toggled.pressed.image.color", theme->titlebut_focused_pressed_toggled_color,
             RrColorCopy(theme->titlebut_focused_pressed_color));

  READ_COLOR("window.inactive.button.toggled.pressed.image.color", theme->titlebut_unfocused_pressed_toggled_color,
             RrColorCopy(theme->titlebut_unfocused_pressed_color));

  READ_COLOR("menu.title.text.color", theme->menu_title_color, RrColorNew(inst, 0, 0, 0));

  READ_COLOR("menu.items.text.color", theme->menu_color, RrColorNew(inst, 0xff, 0xff, 0xff));

  READ_COLOR("menu.bullet.image.color", theme->menu_bullet_color, RrColorCopy(theme->menu_color));

  READ_COLOR("menu.items.disabled.text.color", theme->menu_disabled_color, RrColorNew(inst, 0, 0, 0));

  READ_COLOR("menu.items.active.disabled.text.color", theme->menu_disabled_selected_color,
             RrColorCopy(theme->menu_disabled_color));

  READ_COLOR("menu.items.active.text.color", theme->menu_selected_color, RrColorNew(inst, 0, 0, 0));

  READ_COLOR("menu.separator.color", theme->menu_sep_color, RrColorCopy(theme->menu_color));

  READ_COLOR("menu.bullet.selected.image.color", theme->menu_bullet_selected_color,
             RrColorCopy(theme->menu_selected_color));

  READ_COLOR("osd.button.unpressed.text.color", theme->osd_unpressed_color, RrColorCopy(theme->osd_text_active_color));
  READ_COLOR("osd.button.pressed.text.color", theme->osd_pressed_color, RrColorCopy(theme->osd_text_active_color));
  READ_COLOR("osd.button.focused.text.color", theme->osd_focused_color, RrColorCopy(theme->osd_text_active_color));
  ...
  /* hover */
  READ_APPEARANCE_COPY("window.active.button.hover.bg", fbs.focused_hover, TRUE, fbs.focused_unpressed);
  READ_APPEARANCE_COPY("window.inactive.button.hover.bg", fbs.unfocused_hover, TRUE, fbs.unfocused_unpressed);

  /* toggled unpressed */
  READ_APPEARANCE_("window.active.button.toggled.unpressed.bg", "window.active.button.toggled.bg",
                   fbs.focused_unpressed_toggled, TRUE, fbs.focused_pressed);
  READ_APPEARANCE_("window.inactive.button.toggled.unpressed.bg", "window.inactive.button.toggled.bg",
                   fbs.unfocused_unpressed_toggled, TRUE, fbs.unfocused_pressed);

  /* toggled pressed */
  READ_APPEARANCE_COPY("window.active.button.toggled.pressed.bg", fbs.focused_pressed_toggled, TRUE,
                       fbs.focused_pressed);
  READ_APPEARANCE_COPY("window.inactive.button.toggled.pressed.bg", fbs.unfocused_pressed_toggled, TRUE,
                       fbs.unfocused_pressed);

  /* toggled hover */
  READ_APPEARANCE_COPY("window.active.button.toggled.hover.bg", fbs.focused_hover_toggled, TRUE,
                       fbs.focused_unpressed_toggled);
  READ_APPEARANCE_COPY("window.inactive.button.toggled.hover.bg", fbs.unfocused_hover_toggled, TRUE,
  ...
  /* max button */
  {
    ...
    read_button_styles(db, inst, path, theme, theme->btn_max, "max", &fbs, normal_mask, toggled_mask);
  }

  /* close button */
  {
    guchar normal_mask[] = {0x33, 0x3f, 0x1e, 0x1e, 0x3f, 0x33};
    read_button_styles(db, inst, path, theme, theme->btn_close, "close", &fbs, normal_mask, NULL);
  }
  ...
  /* all desktops button */
  {
    ...
    read_button_styles(db, inst, path, theme, theme->btn_desk, "desk", &fbs, normal_mask, toggled_mask);
  }

  /* shade button */
  {
    guchar normal_mask[] = {0x3f, 0x3f, 0x00, 0x00, 0x00, 0x00};
    read_button_styles(db, inst, path, theme, theme->btn_shade, "shade", &fbs, normal_mask, normal_mask);
  }

  /* iconify button */
  {
    guchar normal_mask[] = {0x00, 0x00, 0x00, 0x00, 0x3f, 0x3f};
    read_button_styles(db, inst, path, theme, theme->btn_iconify, "iconify", &fbs, normal_mask, NULL);
  }
  ...
  theme->def_win_icon = read_c_image(OB_DEFAULT_ICON_WIDTH, OB_DEFAULT_ICON_HEIGHT, OB_DEFAULT_ICON_pixel_data);
  ...
  if (!read_appearance(db, inst, "osd.active.label.bg", theme->osd_hilite_label, TRUE) &&
      !read_appearance(db, inst, "osd.label.bg", theme->osd_hilite_label, TRUE)) {
    RrAppearanceFree(theme->osd_hilite_label);
    theme->osd_hilite_label = RrAppearanceCopy(theme->a_focused_label);
  }
  if (!read_appearance(db, inst, "osd.inactive.label.bg", theme->osd_unhilite_label, TRUE)) {
  ...
  if (!read_appearance(db, inst, "osd.hilight.bg", theme->osd_hilite_bg, FALSE)) {
  ...
  if (!read_appearance(db, inst, "osd.unhilight.bg", theme->osd_unhilite_bg, FALSE)) {
  ...
  theme->a_icon->surface.grad = theme->a_clear->surface.grad = theme->a_clear_tex->surface.grad =
      theme->a_menu_text_title->surface.grad = theme->a_menu_normal->surface.grad =
          theme->a_menu_disabled->surface.grad = theme->a_menu_text_normal->surface.grad =
              theme->a_menu_text_selected->surface.grad = theme->a_menu_text_disabled->surface.grad =
                  theme->a_menu_text_disabled_selected->surface.grad = theme->a_menu_bullet_normal->surface.grad =
  ...
  theme->a_focused_label->texture[0].data.text.color = theme->title_focused_color;
  ...
  if (read_string(db, "osd.active.label.text.font", &str) || read_string(db, "osd.label.text.font", &str)) {
    char* p;
    gint i = 0;
    gint j;
    if (strstr(str, "shadow=y")) {
    ...
    if (theme->title_focused_shadow_color)
      theme->osd_text_active_shadow_color = RrColorCopy(theme->title_focused_shadow_color);
    else
      theme->osd_text_active_shadow_color = RrColorNew(inst, 0, 0, 0);
    theme->osd_text_active_shadow_alpha = theme->title_focused_shadow_alpha;
  }

  theme->osd_hilite_label->texture[0].data.text.shadow_color = theme->osd_text_active_shadow_color;
  theme->osd_hilite_label->texture[0].data.text.shadow_alpha = theme->osd_text_active_shadow_alpha;

  theme->osd_unpressed_button->texture[0].type = theme->osd_pressed_button->texture[0].type =
      theme->osd_focused_button->texture[0].type = RR_TEXTURE_TEXT;

  theme->osd_unpressed_button->texture[0].data.text.justify = theme->osd_pressed_button->texture[0].data.text.justify =
      theme->osd_focused_button->texture[0].data.text.justify = RR_JUSTIFY_CENTER;

  theme->osd_unpressed_button->texture[0].data.text.font = theme->osd_pressed_button->texture[0].data.text.font =
      theme->osd_focused_button->texture[0].data.text.font = theme->osd_font_hilite;

  theme->osd_unpressed_button->texture[0].data.text.color = theme->osd_unpressed_color;
  theme->osd_pressed_button->texture[0].data.text.color = theme->osd_pressed_color;
  theme->osd_focused_button->texture[0].data.text.color = theme->osd_focused_color;

  theme->osd_pressed_button->texture[1].data.lineart.color = theme->osd_pressed_button->texture[2].data.lineart.color =
      theme->osd_pressed_button->texture[3].data.lineart.color =
          theme->osd_pressed_button->texture[4].data.lineart.color = theme->osd_pressed_lineart;

  theme->osd_focused_button->texture[1].data.lineart.color = theme->osd_focused_button->texture[2].data.lineart.color =
      theme->osd_focused_button->texture[3].data.lineart.color =
          theme->osd_focused_button->texture[4].data.lineart.color = theme->osd_focused_lineart;
  ...
  theme->a_unfocused_label->texture[0].data.text.shadow_color = theme->title_unfocused_shadow_color;
  theme->a_unfocused_label->texture[0].data.text.shadow_alpha = theme->title_unfocused_shadow_alpha;
  ...
  theme->osd_unhilite_label->texture[0].data.text.font = theme->osd_font_unhilite;
  theme->osd_unhilite_label->texture[0].data.text.color = theme->osd_text_inactive_color;
  ...
  if (read_string(db, "osd.inactive.label.text.font", &str)) {
    ...
    if (theme->title_unfocused_shadow_color)
      theme->osd_text_inactive_shadow_color = RrColorCopy(theme->title_unfocused_shadow_color);
    else
      theme->osd_text_inactive_shadow_color = RrColorNew(inst, 0, 0, 0);
    theme->osd_text_inactive_shadow_alpha = theme->title_unfocused_shadow_alpha;
  }

  theme->osd_unhilite_label->texture[0].data.text.shadow_color = theme->osd_text_inactive_shadow_color;
  theme->osd_unhilite_label->texture[0].data.text.shadow_alpha = theme->osd_text_inactive_shadow_alpha;
  ...
  if (read_string(db, "menu.title.text.font", &str)) {
    ...
    theme->a_menu_text_title->texture[0].data.text.shadow_color = theme->menu_title_shadow_color;
    theme->a_menu_text_title->texture[0].data.text.shadow_alpha = i;
  }

  theme->a_menu_text_normal->texture[0].type = theme->a_menu_text_selected->texture[0].type =
      theme->a_menu_text_disabled->texture[0].type = theme->a_menu_text_disabled_selected->texture[0].type =
          RR_TEXTURE_TEXT;
  theme->a_menu_text_normal->texture[0].data.text.justify = theme->a_menu_text_selected->texture[0].data.text.justify =
      theme->a_menu_text_disabled->texture[0].data.text.justify =
          theme->a_menu_text_disabled_selected->texture[0].data.text.justify = RR_JUSTIFY_LEFT;
  theme->a_menu_text_normal->texture[0].data.text.font = theme->a_menu_text_selected->texture[0].data.text.font =
      theme->a_menu_text_disabled->texture[0].data.text.font =
          theme->a_menu_text_disabled_selected->texture[0].data.text.font = theme->menu_font;
  theme->a_menu_text_normal->texture[0].data.text.color = theme->menu_color;
  theme->a_menu_text_selected->texture[0].data.text.color = theme->menu_selected_color;
  theme->a_menu_text_disabled->texture[0].data.text.color = theme->menu_disabled_color;
  theme->a_menu_text_disabled_selected->texture[0].data.text.color = theme->menu_disabled_selected_color;
  ...
  if (read_string(db, "menu.items.font", &str)) {
    ...
    if (strstr(str, "shadow=y")) {
      if ((p = strstr(str, "shadowoffset=")))
        ...
        i = 1;
      theme->a_menu_text_normal->texture[0].data.text.shadow_offset_x = i;
      theme->a_menu_text_normal->texture[0].data.text.shadow_offset_y = i;
      theme->a_menu_text_selected->texture[0].data.text.shadow_offset_x = i;
      theme->a_menu_text_selected->texture[0].data.text.shadow_offset_y = i;
      theme->a_menu_text_disabled->texture[0].data.text.shadow_offset_x = i;
      theme->a_menu_text_disabled->texture[0].data.text.shadow_offset_y = i;
      theme->a_menu_text_disabled_selected->texture[0].data.text.shadow_offset_x = i;
      theme->a_menu_text_disabled_selected->texture[0].data.text.shadow_offset_y = i;
    }
    if ((p = strstr(str, "shadowtint="))) {
      i = parse_inline_number(p + strlen("shadowtint="));
      j = (i > 0 ? 0 : 255);
    ...
    theme->a_menu_text_normal->texture[0].data.text.shadow_color =
        theme->a_menu_text_selected->texture[0].data.text.shadow_color =
            theme->a_menu_text_disabled->texture[0].data.text.shadow_color =
                theme->a_menu_text_disabled_selected->texture[0].data.text.shadow_color = theme->menu_text_shadow_color;
    ...
    theme->a_menu_text_normal->texture[0].data.text.shadow_alpha =
        theme->a_menu_text_selected->texture[0].data.text.shadow_alpha =
            theme->a_menu_text_disabled->texture[0].data.text.shadow_alpha =
                theme->a_menu_text_disabled_selected->texture[0].data.text.shadow_alpha = i;
  }

  theme->a_menu_bullet_normal->texture[0].type = theme->a_menu_bullet_selected->texture[0].type = RR_TEXTURE_MASK;
  theme->a_menu_bullet_normal->texture[0].data.mask.mask = theme->a_menu_bullet_selected->texture[0].data.mask.mask =
      theme->menu_bullet_mask;
  theme->a_menu_bullet_normal->texture[0].data.mask.color = theme->menu_bullet_color;
  theme->a_menu_bullet_selected->texture[0].data.mask.color = theme->menu_bullet_selected_color;
  ...
  theme->win_font_height =
      MAX(theme->win_font_height,
          RrFontHeight(theme->win_font_focused, theme->a_unfocused_label->texture[0].data.text.shadow_offset_y));
  theme->menu_title_font_height =
      RrFontHeight(theme->menu_title_font, theme->a_menu_text_title->texture[0].data.text.shadow_offset_y);
  theme->menu_font_height =
      RrFontHeight(theme->menu_font, theme->a_menu_text_normal->texture[0].data.text.shadow_offset_y);
...
static XrmDatabase loaddb(const gchar* name, gchar** path) {
  ...
  if (name[0] == '/') {
    ...
    s = g_build_filename(g_get_home_dir(), ".themes", name, "openbox-3", "themerc", NULL);
    if ((db = XrmGetFileDatabase(s)))
      *path = g_path_get_dirname(s);
    g_free(s);

    for (it = obt_paths_data_dirs(p); !db && it; it = g_slist_next(it)) {
      s = g_build_filename(it->data, "themes", name, "openbox-3", "themerc", NULL);
      if ((db = XrmGetFileDatabase(s)))
        *path = g_path_get_dirname(s);
      g_free(s);
    }
...
static gboolean read_int(XrmDatabase db, const gchar* rname, gint* value) {
  ...
  if (XrmGetResource(db, rname, rclass, &rettype, &retvalue) && retvalue.addr != NULL) {
...
static void parse_appearance(gchar* tex,
  ...
  if (strstr(tex, "sunken") != NULL)
    ...
    *relief = (*grad == RR_SURFACE_PARENTREL) ? RR_RELIEF_FLAT : RR_RELIEF_RAISED;
...
static gboolean read_appearance(XrmDatabase db,
  ...
  if (XrmGetResource(db, rname, rclass, &rettype, &retvalue) && retvalue.addr != NULL) {
    parse_appearance(retvalue.addr, &value->surface.grad, &value->surface.relief, &value->surface.bevel,
    ...
    if (value->surface.grad == RR_SURFACE_SPLIT_VERTICAL) {
      ...
      if (!read_color(db, inst, ctosplitname, &value->surface.split_secondary)) {
...
static RrPixel32* read_c_image(gint width, gint height, const guint8* data) {
  ...
  for (i = 0; i < width * height; ++i) {
    ...
    *p = (((guint)r << RrDefaultRedOffset) + ((guint)g << RrDefaultGreenOffset) + ((guint)b << RrDefaultBlueOffset) +
          ((guint)a << RrDefaultAlphaOffset));
    p++;
  }
...
#define READ_BUTTON_APPEARANCE(typedots, type, fallback)                                                   \
  ...
  btn->a_unfocused_##type->texture[0].typ##e = btn->a_focused_##type->texture[0].typ##e = RR_TEXTURE_MASK; \
  btn->a_unfocused_##type->texture[0].data.mask.mask = btn->a_focused_##type->texture[0].data.mask.mask =  \
      btn->type##_mask;                                                                                    \
  btn->a_unfocused_##type->texture[0].data.mask.color = btn->unfocused_##type##_color;                     \
  btn->a_focused_##type->texture[0].data.mask.color = btn->focused_##type##_color;
