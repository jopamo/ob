...
RrPixmapMask* RrPixmapMaskNew(const RrInstance* inst, gint w, gint h, const gchar* data) {
  ...
  m->mask = XCreateBitmapFromData(RrDisplay(inst), RrRootWindow(inst), data, w, h);
  return m;
}
...
void RrPixmapMaskDraw(Pixmap p, const RrTextureMask* m, const RrRect* area) {
  ...
  XFillRectangle(RrDisplay(m->mask->inst), p, RrColorGC(m->color), x, y, x + m->mask->width, y + m->mask->height);
...
RrPixmapMask* RrPixmapMaskCopy(const RrPixmapMask* src) {
  ...
  m->mask = XCreateBitmapFromData(RrDisplay(m->inst), RrRootWindow(m->inst), m->data, m->width, m->height);
  return m;
}
