...
Pixmap RrPaintPixmap(RrAppearance* a, gint w, gint h) {
  ...
  if (a->surface.grad == RR_SURFACE_PARENTREL &&
      (a->surface.parentx >= a->surface.parent->w || a->surface.parenty >= a->surface.parent->h)) {
    return None;
  }
  ...
  for (i = 0; i < a->textures; i++) {
    switch (a->texture[i].type) {
      ...
      case RR_TEXTURE_LINE_ART:
        ...
        XDrawLine(RrDisplay(a->inst), a->pixmap, RrColorGC(a->texture[i].data.lineart.color),
                  a->texture[i].data.lineart.x1, a->texture[i].data.lineart.y1, a->texture[i].data.lineart.x2,
      ...
      case RR_TEXTURE_IMAGE:
        g_assert(!transferred);
        {
          ...
          RrImageDrawImage(a->surface.pixel_data, &a->texture[i].data.image, a->w, a->h, &narea);
        }
        force_transfer = 1;
        break;
      case RR_TEXTURE_RGBA:
        g_assert(!transferred);
        {
          ...
          RrImageDrawRGBA(a->surface.pixel_data, &a->texture[i].data.rgba, a->w, a->h, &narea);
        }
        force_transfer = 1;
        break;
  ...
  if (!transferred) {
    transferred = 1;
    if ((a->surface.grad != RR_SURFACE_SOLID) || (a->surface.interlaced) || force_transfer) {
      pixel_data_to_pixmap(a, 0, 0, w, h);
    }
  }
...
RrAppearance* RrAppearanceNew(const RrInstance* inst, gint numtex) {
  ...
  if (numtex)
    out->texture = g_new0(RrTexture, numtex);
...
void RrAppearanceAddTextures(RrAppearance* a, gint numtex) {
  ...
  a->textures = numtex;
  if (numtex)
    a->texture = g_new0(RrTexture, numtex);
}
...
RrAppearance* RrAppearanceCopy(RrAppearance* orig) {
  ...
  spc->bevel = spo->bevel;
  if (spo->primary != NULL)
    spc->primary = RrColorNew(copy->inst, spo->primary->r, spo->primary->g, spo->primary->b);
  else
    spc->primary = NULL;

  if (spo->secondary != NULL)
    spc->secondary = RrColorNew(copy->inst, spo->secondary->r, spo->secondary->g, spo->secondary->b);
  else
    spc->secondary = NULL;

  if (spo->border_color != NULL)
    spc->border_color = RrColorNew(copy->inst, spo->border_color->r, spo->border_color->g, spo->border_color->b);
  else
    spc->border_color = NULL;

  if (spo->interlace_color != NULL)
    spc->interlace_color =
        RrColorNew(copy->inst, spo->interlace_color->r, spo->interlace_color->g, spo->interlace_color->b);
  else
    spc->interlace_color = NULL;

  if (spo->bevel_dark != NULL)
    spc->bevel_dark = RrColorNew(copy->inst, spo->bevel_dark->r, spo->bevel_dark->g, spo->bevel_dark->b);
  else
    spc->bevel_dark = NULL;

  if (spo->bevel_light != NULL)
    spc->bevel_light = RrColorNew(copy->inst, spo->bevel_light->r, spo->bevel_light->g, spo->bevel_light->b);
  else
    spc->bevel_light = NULL;

  if (spo->split_primary != NULL)
    spc->split_primary = RrColorNew(copy->inst, spo->split_primary->r, spo->split_primary->g, spo->split_primary->b);
  else
    spc->split_primary = NULL;

  if (spo->split_secondary != NULL)
    spc->split_secondary =
        RrColorNew(copy->inst, spo->split_secondary->r, spo->split_secondary->g, spo->split_secondary->b);
  else
    spc->split_secondary = NULL;
...
void RrAppearanceFree(RrAppearance* a) {
  if (a) {
    RrSurface* p;
    if (a->pixmap != None)
      XFreePixmap(RrDisplay(a->inst), a->pixmap);
    if (a->xftdraw != NULL)
      XftDrawDestroy(a->xftdraw);
    if (a->textures)
      g_free(a->texture);
...
static void pixel_data_to_pixmap(RrAppearance* l, gint x, gint y, gint w, gint h) {
  ...
  Pixmap out;
  XImage* im = NULL;
  im = XCreateImage(RrDisplay(l->inst), RrVisual(l->inst), RrDepth(l->inst), ZPixmap, 0, NULL, w, h, 32, 0);
  g_assert(im != NULL);
  ...
  for (i = 0; i < a->textures; ++i) {
    switch (a->texture[i].type) {
      ...
      case RR_TEXTURE_IMAGE:
        ...
        break;
      case RR_TEXTURE_LINE_ART:
        w = MAX(w, MAX(a->texture[i].data.lineart.x1 - l - r, a->texture[i].data.lineart.x2 - l - r));
        break;
      case RR_TEXTURE_NUM_TYPES:
        g_assert_not_reached();
    }
  ...
  for (i = 0; i < a->textures; ++i) {
    switch (a->texture[i].type) {
      ...
      case RR_TEXTURE_IMAGE:
        ...
        break;
      case RR_TEXTURE_LINE_ART:
        h = MAX(h, MAX(a->texture[i].data.lineart.y1 - t - b, a->texture[i].data.lineart.y2 - t - b));
        break;
      case RR_TEXTURE_NUM_TYPES:
        g_assert_not_reached();
    }
...
static void reverse_bits(gchar* c, gint n) {
  gint i;
  for (i = 0; i < n; i++, c++)
    *c = (((*c * 0x0802UL & 0x22110UL) | (*c * 0x8020UL & 0x88440UL)) * 0x10101UL) >> 16;
}

gboolean RrPixmapToRGBA(const RrInstance* inst, Pixmap pmap, Pixmap mask, gint* w, gint* h, RrPixel32** data) {
  Window xr;
  gint xx, xy;
  guint pw, ph, mw, mh, xb, xd, i, x, y, di;
  XImage *xi, *xm = NULL;

  if (!XGetGeometry(RrDisplay(inst), pmap, &xr, &xx, &xy, &pw, &ph, &xb, &xd))
    return FALSE;

  if (mask) {
    if (!XGetGeometry(RrDisplay(inst), mask, &xr, &xx, &xy, &mw, &mh, &xb, &xd))
      return FALSE;
    if (pw != mw || ph != mh || xd != 1)
      return FALSE;
  }

  xi = XGetImage(RrDisplay(inst), pmap, 0, 0, pw, ph, 0xffffffff, ZPixmap);
  if (!xi)
    return FALSE;

  if (mask) {
    xm = XGetImage(RrDisplay(inst), mask, 0, 0, mw, mh, 0xffffffff, ZPixmap);
    if (!xm) {
      XDestroyImage(xi);
      return FALSE;
    }
