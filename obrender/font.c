...
static void measure_font(const RrInstance* inst, RrFont* f) {
  ...
  if (lang == NULL) {
#if PANGO_VERSION_MAJOR > 1 || (PANGO_VERSION_MAJOR == 1 && PANGO_VERSION_MINOR >= 16)
...
RrFont* RrFontOpen(const RrInstance* inst, const gchar* name, gint size, RrFontWeight weight, RrFontSlant slant) {
  RrFont* out;
  PangoWeight pweight;
  PangoStyle pstyle;
  ...
  switch (weight) {
    case RR_FONTWEIGHT_LIGHT:
      pweight = PANGO_WEIGHT_LIGHT;
      break;
    case RR_FONTWEIGHT_NORMAL:
      pweight = PANGO_WEIGHT_NORMAL;
      break;
    case RR_FONTWEIGHT_SEMIBOLD:
      pweight = PANGO_WEIGHT_SEMIBOLD;
      break;
    case RR_FONTWEIGHT_BOLD:
      pweight = PANGO_WEIGHT_BOLD;
      break;
    case RR_FONTWEIGHT_ULTRABOLD:
      pweight = PANGO_WEIGHT_ULTRABOLD;
      break;
    default:
  ...
  switch (slant) {
    case RR_FONTSLANT_NORMAL:
      pstyle = PANGO_STYLE_NORMAL;
      break;
    case RR_FONTSLANT_ITALIC:
      pstyle = PANGO_STYLE_ITALIC;
      break;
    case RR_FONTSLANT_OBLIQUE:
...
RrFont* RrFontOpenDefault(const RrInstance* inst) {
  return RrFontOpen(inst, RrDefaultFontFamily, RrDefaultFontSize, RrDefaultFontWeight, RrDefaultFontSlant);
}
...
static void font_measure_full(const RrFont* f,
                              const gchar* str,
                              gint* x,
                              gint* y,
                              gint shadow_x,
...
RrSize* RrFontMeasureString(const RrFont* f,
                            const gchar* str,
                            gint shadow_x,
                            gint shadow_y,
                            gboolean flow,
  ...
  font_measure_full(f, str, &size->width, &size->height, shadow_x, shadow_y, flow, maxwidth);
  return size;
}
...
void RrFontDraw(XftDraw* d, RrTextureText* t, RrRect* area) {
  ...
  if (t->shadow_offset_x || t->shadow_offset_y) {
    ...
    */
    ...
    /* see below... */
    if (!t->flow) {
      pango_xft_render_layout_line(d, &c,
#if PANGO_VERSION_MAJOR > 1 || (PANGO_VERSION_MAJOR == 1 && PANGO_VERSION_MINOR >= 16)
                                   pango_layout_get_line_readonly(t->font->layout, 0),
#else
                                   pango_layout_get_line(t->font->layout, 0),
#endif
                                   (x + t->shadow_offset_x) * PANGO_SCALE, (y + t->shadow_offset_y) * PANGO_SCALE);
    }
    else {
      pango_xft_render_layout(d, &c, t->font->layout, (x + t->shadow_offset_x) * PANGO_SCALE,
  ...
  if (t->shortcut) {
    ...
    t->font->shortcut_underline->end_index = t->shortcut_pos + (g_utf8_next_char(s) - s);
  ...
  if (!t->flow) {
    ...
    pango_xft_render_layout(d, &c, t->font->layout, x * PANGO_SCALE, y * PANGO_SCALE);
  }
