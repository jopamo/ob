...
#include "color.h"
#endif
static void dest(gpointer data) {
#ifdef DEBUG
  RrColor* c = data;
  if (c->refcount > 0)
    g_error(
        "color %d (%d,%d,%d) in hash table with %d "
        "leftover references",
...
static void RrTrueColorSetup(RrInstance* inst) {
  ...
  timage = XCreateImage(inst->display, inst->visual, inst->depth, ZPixmap, 0, NULL, 1, 1, 32, 0);
...
static void RrPseudoColorSetup(RrInstance* inst) {
  ...
  for (i = 0; i < _ncolors; i++)
    if (!XAllocColor(inst->display, inst->colormap, &inst->pseudo_colors[i]))
