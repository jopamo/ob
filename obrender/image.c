 ...
 */
static void RrImageSetRemovePictureAt(RrImageSet* self, gint i, gboolean original) {
  RrImagePic*** list;
  gint* len;
...
/*! Add an RrImagePic to an RrImageSet.
  ...
  newest to oldest.
*/
static void RrImageSetAddPicture(RrImageSet* self, RrImagePic* pic, gboolean original) {
  gint i;
  RrImagePic*** list;
  gint* len;
  ...
  if (original) {
    ...
    for (i = 0; i < self->n_resized; ++i)
      if (self->resized[i]->width == pic->width || self->resized[i]->height == pic->height) {
...
void RrImageSetDestroyFunc(RrImage* self, RrImageDestroyFunc func, gpointer data) {
...
RrImage* RrImageNewFromData(RrImageCache* cache, RrPixel32* data, gint w, gint h) {
  RrImagePic pic, *ppic;
  RrImage* self;
  RrImageSet* set;
...
ImlibLoader* LoadWithImlib(gchar* path, RrPixel32** pixel_data, gint* width, gint* height) {
...
*/
static RrImagePic* ResizeImage(RrPixel32* src, gulong srcW, gulong srcH, gulong dstW, gulong dstH) {
...
void DrawRGBA(RrPixel32* target,
              gint target_w,
              gint target_h,
              RrPixel32* source,
              gint source_w,
  ...
  col = 0;
  num_pixels = dw * dh;
  dest = target + area->x + (area->width - dw) / 2 + (target_w * (area->y + (area->height - dh) / 2));
  while (num_pixels-- > 0) {
...
void RrImageDrawRGBA(RrPixel32* target, RrTextureRGBA* rgba, gint target_w, gint target_h, RrRect* area) {
  ...
  scaled = ResizeImage(rgba->data, rgba->width, rgba->height, area->width, area->height);

  if (scaled) {
#ifdef DEBUG
    g_warning(
        "Scaling an RGBA! You should avoid this and just make "
        "it the right size yourself!");
#endif
    DrawRGBA(target, target_w, target_h, scaled->data, scaled->width, scaled->height, rgba->alpha, area);
    RrImagePicFree(scaled);
  }
  else
    DrawRGBA(target, target_w, target_h, rgba->data, rgba->width, rgba->height, rgba->alpha, area);
}
...
void RrImageDrawImage(RrPixel32* target, RrTextureImage* img, gint target_w, gint target_h, RrRect* area) {
  ...
  for (i = 0; i < set->n_original; ++i)
    if ((set->original[i]->width >= set->original[i]->height && set->original[i]->width == area->width) ||
        (set->original[i]->width <= set->original[i]->height && set->original[i]->height == area->height)) {
      pic = set->original[i];
      break;
    }
  ...
  for (i = 0; i < set->n_resized; ++i)
    if ((set->resized[i]->width >= set->resized[i]->height && set->resized[i]->width == area->width) ||
        (set->resized[i]->width <= set->resized[i]->height && set->resized[i]->height == area->height)) {
      gint j;
      RrImagePic* saved;
  ...
  if (!pic) {
    ...
    for (i = 0; i < set->n_original; ++i) {
      ...
      if (wdiff < 0)
        wdiff *= 2; /* prefer scaling down than up */
      hdiff = set->original[i]->height - area->height;
      if (hdiff < 0)
        hdiff *= 2; /* prefer scaling down than up */
      ...
      myasp = ((gdouble)set->original[i]->width) / set->original[i]->height;
      if (ABS(aspect - myasp) < 0.0000001 && (min_aspect_diff < 0 || diff < min_aspect_diff)) {
    ...
    pic = ResizeImage(set->original[min_i]->data, set->original[min_i]->width, set->original[min_i]->height,
  ...
  DrawRGBA(target, target_w, target_h, pic->data, pic->width, pic->height, img->alpha, area);
  if (free_pic)
    RrImagePicFree(pic);
}
