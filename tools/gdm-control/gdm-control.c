...
typedef enum { INVALID, NONE, SHUTDOWN, REBOOT, SUSPEND, SWITCHUSER } Action;
...
static char* gdm_send_protocol_msg(const char* msg) {
  ...
  if (write(fd, p, strlen(p)) < 0) {
    ...
    g_warning("Failed to send message to GDM: %s", g_strerror(errno));
    return NULL;
  }
  g_free(p);
...
static gboolean gdm_authenticate() {
  ...
  while ((xau = XauReadAuth(f))) {
    ...
    if (xau->family != FamilyLocal || strncmp(xau->number, display_number, xau->number_length) ||
        strncmp(xau->name, "MIT-MAGIC-COOKIE-1", xau->name_length) || xau->data_length != GDM_MIT_MAGIC_COOKIE_LEN) {
      XauDisposeAuth(xau);
      continue;
    }
