...
typedef enum { OB_CYCLE_NONE = 0, OB_CYCLE_NORMAL, OB_CYCLE_DIRECTIONAL } ObCycleType;
...
static ObClient* focus_find_directional(ObClient* c, ObDirection dir, gboolean dock_windows, gboolean desktop_windows);
...
void focus_cycle_reorder() {
  if (focus_cycle_type == OB_CYCLE_NORMAL) {
    focus_cycle_target = focus_cycle_popup_refresh(focus_cycle_target, TRUE, focus_cycle_linear);
    focus_cycle_update_indicator(focus_cycle_target);
    if (!focus_cycle_target)
      focus_cycle(TRUE, TRUE, TRUE, TRUE, TRUE, TRUE, TRUE, OB_FOCUS_CYCLE_POPUP_MODE_NONE, TRUE, TRUE);
  }
}

ObClient* focus_cycle(gboolean forward,
...
static ObClient* focus_find_directional(ObClient* c, ObDirection dir, gboolean dock_windows, gboolean desktop_windows) {
  ...
  for (it = g_list_first(client_list); it; it = g_list_next(it)) {
    ...
    if (dir == OB_DIRECTION_NORTHEAST || dir == OB_DIRECTION_SOUTHEAST || dir == OB_DIRECTION_SOUTHWEST ||
    ...
    switch (dir) {
      ...
      case OB_DIRECTION_SOUTHWEST:
        ...
        distance = ((dir == OB_DIRECTION_NORTH || dir == OB_DIRECTION_NORTHEAST) ? -his_cy : his_cy);
        break;
      ...
      case OB_DIRECTION_NORTHWEST:
        ...
        distance = ((dir == OB_DIRECTION_WEST || dir == OB_DIRECTION_NORTHWEST) ? -his_cx : his_cx);
        break;
    }
...
ObClient* focus_directional_cycle(ObDirection dir,
                                  gboolean dock_windows,
                                  gboolean desktop_windows,
  ...
  if (focus_cycle_target)
    ft = focus_find_directional(focus_cycle_target, dir, dock_windows, desktop_windows);
  else if (first)
...
gboolean focus_cycle_valid(struct _ObClient* client) {
  return focus_valid_target(client, screen_desktop, TRUE, focus_cycle_iconic_windows, focus_cycle_all_desktops,
                            focus_cycle_nonhilite_windows, focus_cycle_dock_windows, focus_cycle_desktop_windows,
                            FALSE);
}
