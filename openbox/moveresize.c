...
static void popup_coords(ObClient* c, const gchar* format, gint a, gint b) {
  ...
  if (config_resize_popup_pos == OB_RESIZE_POS_TOP)
    popup_position(popup, SouthGravity, c->frame->area.x + c->frame->area.width / 2,
                   c->frame->area.y - ob_rr_theme->fbwidth);
  else if (config_resize_popup_pos == OB_RESIZE_POS_CENTER)
    popup_position(popup, CenterGravity, c->frame->area.x + c->frame->area.width / 2,
...
void moveresize_start(ObClient* c, gint x, gint y, guint b, guint32 cnr) {
  ObCursor cur;
  gboolean mv = (cnr == OBT_PROP_ATOM(NET_WM_MOVERESIZE_MOVE) || cnr == OBT_PROP_ATOM(NET_WM_MOVERESIZE_MOVE_KEYBOARD));
  gint up = 1;
  gint left = 1;

  if (moveresize_in_progress || !c->frame->visible ||
      !(mv ? (c->functions & OB_CLIENT_FUNC_MOVE) : (c->functions & OB_CLIENT_FUNC_RESIZE)))
    return;
...
#ifdef SYNC
  if (config_resize_redraw && !moving && obt_display_extension_sync && moveresize_client->sync_request &&
      moveresize_client->sync_counter && !moveresize_client->not_responding) {
...
void moveresize_end(gboolean cancel) {
  ...
  */
  client_configure(moveresize_client, (cancel ? start_cx : cur_x), (cancel ? start_cy : cur_y),
                   (cancel ? start_cw : cur_w), (cancel ? start_ch : cur_h), TRUE, TRUE, FALSE);
...
static void do_move(gboolean keyboard, gint keydist) {
  ...
  if (keyboard)
    resist = keydist - 1; /* resist for one key press */
  else
  ...
  if (config_resize_popup_show == 2) /* == "Always" */
    popup_coords(moveresize_client, "%d x %d", moveresize_client->frame->area.x, moveresize_client->frame->area.y);
}
...
static void do_resize(void) {
  ...
  if (!(w == moveresize_client->area.width && h == moveresize_client->area.height) &&
  ...
  if (config_resize_popup_show == 2 ||  /* == "Always" */
      (config_resize_popup_show == 1 && /* == "Nonpixel" */
       moveresize_client->size_inc.width > 1 && moveresize_client->size_inc.height > 1))
...
static void calc_resize(gboolean keyboard, gint keydist, gint* dw, gint* dh, ObDirection dir) {
...
static void move_with_keys(KeySym sym, guint state) {
  ...
  if (state & obt_keyboard_modkey_to_modmask(OBT_KEYBOARD_MODKEY_SHIFT)) {
    ...
    if (state & obt_keyboard_modkey_to_modmask(OBT_KEYBOARD_MODKEY_CONTROL)) {
      dist = 1;
    }
    else
      dist = KEY_DIST;
...
static void resize_with_keys(KeySym sym, guint state) {
  ...
  if (sym == XK_Right) {
    dir = OB_DIRECTION_EAST;
    if (key_resize_edge != OB_DIRECTION_WEST && key_resize_edge != OB_DIRECTION_EAST) {
    ...
    dir = OB_DIRECTION_NORTH;
    if (key_resize_edge != OB_DIRECTION_NORTH && key_resize_edge != OB_DIRECTION_SOUTH) {
  ...
  if (state & obt_keyboard_modkey_to_modmask(OBT_KEYBOARD_MODKEY_SHIFT)) {
    ...
    ObClientDirectionalResizeType resize_type = key_resize_edge == dir ? CLIENT_RESIZE_GROW : CLIENT_RESIZE_SHRINK;

    client_find_resize_directional(moveresize_client, key_resize_edge, resize_type, &x, &y, &w, &h);
    ...
    else if (state & obt_keyboard_modkey_to_modmask(OBT_KEYBOARD_MODKEY_CONTROL)) {
      distw = 1;
      resist = 1;
    }
    else {
  ...
  if (moveresize_client->max_horz && (key_resize_edge == OB_DIRECTION_WEST || key_resize_edge == OB_DIRECTION_EAST)) {
    /* unmax horz */
    was_max_horz = TRUE;
    ...
    client_maximize(moveresize_client, FALSE, 1);
           ...
           (key_resize_edge == OB_DIRECTION_NORTH || key_resize_edge == OB_DIRECTION_SOUTH)) {
    /* unmax vert */
    was_max_vert = TRUE;
...
gboolean moveresize_event(XEvent* e) {
  ...
  if (e->type == ButtonPress) {
    ...
    if (moving) {
      ...
      else if (corner == OBT_PROP_ATOM(NET_WM_MOVERESIZE_SIZE_TOPRIGHT)) {
        dw = (e->xmotion.x_root - start_x);
        dh = -(e->xmotion.y_root - start_y);
      ...
      else if (corner == OBT_PROP_ATOM(NET_WM_MOVERESIZE_SIZE_BOTTOMRIGHT)) {
        dw = (e->xmotion.x_root - start_x);
        dh = (e->xmotion.y_root - start_y);
      ...
      else if (corner == OBT_PROP_ATOM(NET_WM_MOVERESIZE_SIZE_BOTTOMLEFT)) {
        dw = -(e->xmotion.x_root - start_x);
        dh = (e->xmotion.y_root - start_y);
      ...
      else if (corner == OBT_PROP_ATOM(NET_WM_MOVERESIZE_SIZE_KEYBOARD)) {
        dw = (e->xmotion.x_root - start_x);
        dh = (e->xmotion.y_root - start_y);
        dir = OB_DIRECTION_SOUTHEAST;
      }
    ...
    if (sym == XK_Escape) {
      ...
      else if (corner == OBT_PROP_ATOM(NET_WM_MOVERESIZE_MOVE_KEYBOARD)) {
