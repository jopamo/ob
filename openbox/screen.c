...
gboolean screen_annex(void) {
  ...
  OBT_PROP_SET32(obt_root(ob_screen), NET_SUPPORTING_WM_CHECK, WINDOW, screen_support_win);
  ...
  */
  ...
  OBT_PROP_SETA32(obt_root(ob_screen), NET_SUPPORTED, ATOM, supported, num_support);
  g_free(supported);

  OBT_PROP_SETS(RootWindow(obt_display, ob_screen), OB_VERSION, OPENBOX_VERSION);
...
static void screen_tell_ksplash(void) {
  ...
  g_spawn_async(
      NULL, argv, NULL,
      G_SPAWN_SEARCH_PATH | G_SPAWN_DO_NOT_REAP_CHILD | G_SPAWN_STDERR_TO_DEV_NULL | G_SPAWN_STDOUT_TO_DEV_NULL, NULL,
      NULL, NULL, NULL);
  g_strfreev(argv);
...
void screen_startup(gboolean reconfig) {
  ...
  */
  if (!namesexist && session_desktop_names != NULL) {
    ...
    OBT_PROP_SETSS(obt_root(ob_screen), NET_DESKTOP_NAMES, (const gchar* const*)names);
  ...
  if (OBT_PROP_GET32(obt_root(ob_screen), NET_NUMBER_OF_DESKTOPS, CARDINAL, &d)) {
    if (d != config_desktops_num) {
      /* TRANSLATORS: If you need to specify a different order of the
         arguments, you can use %1$d for the first one and %2$d for the
         second one. For example,
         "The current session has %2$d desktops, but Openbox is configured for %1$d ..." */
      g_warning(ngettext("Openbox is configured for %d desktop, but the current session has %d.  Overriding the "
                         "Openbox configuration.",
                         "Openbox is configured for %d desktops, but the current session has %d.  Overriding the "
                         "Openbox configuration.",
                         config_desktops_num),
  ...
  if (OBT_PROP_GET32(obt_root(ob_screen), NET_CURRENT_DESKTOP, CARDINAL, &d) && d < screen_num_desktops) {
    screen_set_desktop(d, FALSE);
  }
  else if (session_desktop >= 0)
    screen_set_desktop(MIN((guint)session_desktop, screen_num_desktops), FALSE);
  else
  ...
  OBT_PROP_SET32(obt_root(ob_screen), NET_SHOWING_DESKTOP, CARDINAL, screen_showing_desktop());

  if (session_desktop_layout_present && screen_validate_layout(&session_desktop_layout)) {
...
void screen_resize(void) {
  ...
  OBT_PROP_SETA32(obt_root(ob_screen), NET_DESKTOP_GEOMETRY, CARDINAL, geometry, 2);
...
void screen_set_num_desktops(guint num) {
  ...
  OBT_PROP_SETA32(obt_root(ob_screen), NET_DESKTOP_VIEWPORT, CARDINAL, viewport, num * 2);
  g_free(viewport);
...
void screen_remove_desktop(gboolean current) {
  ...
  for (it = g_list_last(stacking_copy); it; it = g_list_previous(it)) {
    if (WINDOW_IS_CLIENT(it->data)) {
      ...
      /* raise all the windows that are on the current desktop which
         is being merged */
      if ((screen_desktop == rmdesktop - 1 || screen_desktop == rmdesktop) &&
  ...
  switch (screen_desktop_layout.orientation) {
    case OB_ORIENTATION_HORZ:
      switch (screen_desktop_layout.start_corner) {
        ...
        case OB_CORNER_TOPRIGHT:
          return r % screen_desktop_layout.rows * screen_desktop_layout.columns +
    ...
    case OB_ORIENTATION_VERT:
      switch (screen_desktop_layout.start_corner) {
        ...
        case OB_CORNER_BOTTOMLEFT:
          return c % screen_desktop_layout.columns * screen_desktop_layout.rows +
...
void screen_show_desktop_popup(guint d, gboolean perm) {
  ...
  if (!config_desktop_popup_time)
    return;

  a = screen_physical_area_primary(FALSE);
  pager_popup_position(desktop_popup, CenterGravity, a->x + a->width / 2, a->y + a->height / 2);
  pager_popup_icon_size_multiplier(desktop_popup, (screen_desktop_layout.columns / screen_desktop_layout.rows) / 2,
                                   (screen_desktop_layout.rows / screen_desktop_layout.columns) / 2);
  pager_popup_max_width(desktop_popup, MAX(a->width / 3, POPUP_WIDTH));
  pager_popup_show(desktop_popup, screen_desktop_names[d], d);

  if (desktop_popup_timer)
    g_source_remove(desktop_popup_timer);
  desktop_popup_timer = 0;
  if (!perm && !desktop_popup_perm)
    /* only hide if its not already being show permanently */
    desktop_popup_timer = g_timeout_add(config_desktop_popup_time, hide_desktop_popup_func, desktop_popup);
  if (perm)
...
static gboolean screen_validate_layout(ObDesktopLayout* l) {
  ...
  if (l->orientation == OB_ORIENTATION_HORZ) {
    ...
    l->rows = MIN(l->rows, (screen_num_desktops + l->columns - 1) / l->columns);
    l->columns = screen_num_desktops / l->rows + !!(screen_num_desktops % l->rows);
  }
  else {
    l->rows = MIN(screen_num_desktops, l->rows);
    l->columns = MIN(l->columns, (screen_num_desktops + l->rows - 1) / l->rows);
    l->rows = screen_num_desktops / l->columns + !!(screen_num_desktops % l->columns);
  }
  return TRUE;
}
...

{
  ...
  if (OBT_PROP_GETA32(obt_root(ob_screen), NET_DESKTOP_LAYOUT, CARDINAL, &data, &num)) {
    if (num == 3 || num == 4) {
...
void screen_update_desktop_names(void) {
  ...
  if (OBT_PROP_GETSS(obt_root(ob_screen), NET_DESKTOP_NAMES, &screen_desktop_names))
...
void screen_show_desktop(ObScreenShowDestopMode show_mode, ObClient* show_only) {
  ...
  if (screen_show_desktop_mode == SCREEN_SHOW_DESKTOP_UNTIL_TOGGLE && show_only != NULL) {
  ...
  if (showing_after) {
    ...
    for (it = focus_order; it; it = g_list_next(it)) {
      ObClient* c = it->data;
      if (c->type == OB_CLIENT_TYPE_DESKTOP && (c->desktop == screen_desktop || c->desktop == DESKTOP_ALL) &&
  ...
  OBT_PROP_SET32(obt_root(ob_screen), NET_SHOWING_DESKTOP, CARDINAL, !!showing_after);
}
...
static void get_xinerama_screens(Rect** xin_areas, guint* nxin) {
  ...
  if (ob_debug_xinerama) {
...
#ifdef XINERAMA
  else if (obt_display_extension_xinerama && (info = XineramaQueryScreens(obt_display, &n))) {
    *nxin = n;
    *xin_areas = g_new(Rect, *nxin + 1);
    for (i = 0; i < *nxin; ++i)
    ...
    *nxin = 1;
    *xin_areas = g_new(Rect, *nxin + 1);
    RECT_SET((*xin_areas)[0], 0, 0, WidthOfScreen(ScreenOfDisplay(obt_display, ob_screen)),
  ...
  for (i = 0; i < *nxin; ++i)
    ob_debug("Monitor %d @ %d,%d %dx%d\n", i, (*xin_areas)[i].x, (*xin_areas)[i].y, (*xin_areas)[i].width,
             (*xin_areas)[i].height);
  ob_debug("Full desktop @ %d,%d %dx%d\n", (*xin_areas)[i].x, (*xin_areas)[i].y, (*xin_areas)[i].width,
           (*xin_areas)[i].height);
}
...
void screen_update_areas(void) {
  ...
  OBT_PROP_SETA32(obt_root(ob_screen), NET_WORKAREA, CARDINAL, dims, 4 * screen_num_desktops);
...
#define STRUT_LEFT_IN_SEARCH(s, search) \
  (RANGES_INTERSECT(search->y, search->height, s->left_start, s->left_end - s->left_start + 1))
#define STRUT_RIGHT_IN_SEARCH(s, search) \
  (RANGES_INTERSECT(search->y, search->height, s->right_start, s->right_end - s->right_start + 1))
#define STRUT_TOP_IN_SEARCH(s, search) \
  (RANGES_INTERSECT(search->x, search->width, s->top_start, s->top_end - s->top_start + 1))
#define STRUT_BOTTOM_IN_SEARCH(s, search) \
  (RANGES_INTERSECT(search->x, search->width, s->bottom_start, s->bottom_end - s->bottom_start + 1))

#define STRUT_LEFT_IGNORE(s, us, search) \
  (head == SCREEN_AREA_ALL_MONITORS && us && RECT_LEFT(monitor_area[i]) + s->left > RECT_LEFT(*search))
#define STRUT_RIGHT_IGNORE(s, us, search) \
  (head == SCREEN_AREA_ALL_MONITORS && us && RECT_RIGHT(monitor_area[i]) - s->right < RECT_RIGHT(*search))
#define STRUT_TOP_IGNORE(s, us, search) \
  (head == SCREEN_AREA_ALL_MONITORS && us && RECT_TOP(monitor_area[i]) + s->top > RECT_TOP(*search))
#define STRUT_BOTTOM_IGNORE(s, us, search) \
  (head == SCREEN_AREA_ALL_MONITORS && us && RECT_BOTTOM(monitor_area[i]) - s->bottom < RECT_BOTTOM(*search))
...
void screen_set_root_cursor(void) {
  if (sn_app_starting())
    XDefineCursor(obt_display, obt_root(ob_screen), ob_cursor(OB_CURSOR_BUSYPOINTER));
  else
...
gboolean screen_pointer_pos(gint* x, gint* y) {
  ...
  ret = !!XQueryPointer(obt_display, obt_root(ob_screen), &w, &w, x, y, &i, &i, &u);
  if (!ret) {
    for (i = 0; i < ScreenCount(obt_display); ++i)
      if (i != ob_screen)
        if (XQueryPointer(obt_display, obt_root(i), &w, &w, x, y, &i, &i, &u))
          break;
  }
  return ret;
}
