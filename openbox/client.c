...
void client_update_icons(ObClient* self) {
  ...
  if (OBT_PROP_GETA32(self->window, NET_WM_ICON, CARDINAL, &data, &num)) {
    ...
    while (i + 2 < num) { /* +2 is to make sure there is a w and h */
      ...
      for (j = 0; j < w * h; ++j)
        data[i + j] = (((data[i + j] >> 24) & 0xff) << RrDefaultAlphaOffset) +
  ...
  */
  if (!self->icon_set && !self->parents) {
    ...
    for (i = 0; i < w * h; ++i)
      ldata[i + 2] =
          (((icon[i] >> RrDefaultAlphaOffset) & 0xff) << 24) + (((icon[i] >> RrDefaultRedOffset) & 0xff) << 16) +
          (((icon[i] >> RrDefaultGreenOffset) & 0xff) << 8) + (((icon[i] >> RrDefaultBlueOffset) & 0xff) << 0);
