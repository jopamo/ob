...
static void client_set_desktop_recursive(ObClient* self, guint target, gboolean donthide, gboolean dontraise);
static void client_change_allowed_actions(ObClient* self);
static void client_change_state(ObClient* self);
static void client_change_wm_state(ObClient* self);
static void client_apply_startup_state(ObClient* self, gint x, gint y, gint w, gint h);
...
static GSList* client_search_all_top_parents_internal(ObClient* self, gboolean bylayer, ObStackingLayer layer);
...
void client_manage(Window window, ObPrompt* prompt) {
  ...
  ob_debug("Window name: %s class: %s role: %s title: %s", self->name, self->class, self->role, self->title);
  ob_debug("Window group name: %s group class: %s", self->group_name, self->group_class);
  ...
  */
  ...
  if (!screen_compare_desktops(self->desktop, screen_desktop) && focus_client &&
      client_search_transient(focus_client, self) && (client_helper(self) || self->modal)) {
  ...
  if (ob_state() != OB_STATE_STARTING && (!self->session || self->session->focused) &&
      /* this means focus=true for window is same as config_focus_new=true */
      ((config_focus_new || settings->focus == 1) || client_search_focus_tree_full(self)) &&
      /* NET_WM_USER_TIME 0 when mapping means don't focus */
      (user_time != 0) &&
      /* this checks for focus=false for the window */
      settings->focus != 0 &&
      focus_valid_target(self, self->desktop, FALSE, FALSE, TRUE, TRUE, FALSE, FALSE, settings->focus == 1)) {
    try_activate = TRUE;
  }
  ...
  ob_debug("Going to try activate new window? %s", try_activate ? "yes" : "no");
  if (try_activate)
    do_activate = client_can_steal_focus(self, settings->focus == 1, (!!launch_time || settings->focus == 1),
  ...
  if (ob_state() == OB_STATE_RUNNING) {
    ...
    ob_debug(
        ...
        (!self->sized ? "no"
                      : (self->sized == PSize
                             ...
                             : (self->sized == USSize
                                    ? "user specified"
                                    : (self->sized == (PSize | USSize) ? "program + user specified" : "BADNESS !?")))),
    ...
    client_find_onscreen(
        self, &place.x, &place.y, place.width, place.height,
        ...
        */
        ob_state() == OB_STATE_RUNNING &&
            (self->type == OB_CLIENT_TYPE_DIALOG || self->type == OB_CLIENT_TYPE_SPLASH ||
             (!((self->positioned & USPosition) || settings->pos_given) && client_normal(self) && !self->session &&
  ...
  */
  if (ob_state() == OB_STATE_RUNNING && (transient || (!(self->sized & USSize || self->positioned & USPosition) &&
  ...
  ob_debug(
      "placing window 0x%x at %d, %d with size %d x %d. "
      "some restrictions may apply",
      self->window, place.x, place.y, place.width, place.height);
  if (self->session)
    ob_debug(
        "  but session requested %d, %d  %d x %d instead, "
        "overriding",
        self->session->x, self->session->y, self->session->w, self->session->h);
  ...
  */
  ...
  ob_debug("Managed window 0x%lx plate 0x%x (%s)", window, self->frame->window, self->class);
}
...
ObClient* client_fake_manage(Window window) {
  ...
  client_apply_startup_state(self, self->area.x, self->area.y, self->area.width, self->area.height);

  ob_debug("gave extents left %d right %d top %d bottom %d", self->frame->size.left, self->frame->size.right,
...
static gboolean client_can_steal_focus(ObClient* self,
  ...
  relative_focused = (focus_client != NULL &&
                      (client_search_focus_tree_full(self) != NULL || client_search_focus_group_full(self) != NULL));
  ...
  ob_debug(
      ...
      (allow_other_desktop ? "yes" : "no"), screen_desktop_user_time);
  ...
  */
  ...
  if (!launch_time) {
    if (client_has_relative(self)) {
      if (event_last_user_time && client_search_focus_group_full(self)) {
        ...
        ob_debug(
            "Unknown launch time, using %u - window in active "
            "group",
            launch_time);
      }
        ...
        ob_debug(
            "Unknown launch time, using %u - window in inactive "
            "group",
            launch_time);
      }
      else {
        /* has relatives which are not being used, but the user seems
           to want to go there! */
        launch_time = event_last_user_time;
        ob_debug("Unknown launch time, using %u - user request", launch_time);
      }
    }
    else {
      /* the window is on its own, probably the user knows it is going
         to appear */
      launch_time = event_last_user_time;
      ob_debug("Unknown launch time, using %u - independent window", launch_time);
    }
  }
  ...
  /* If something is focused... */
    ...
    /* If the user is working in another window right now, then don't
       steal focus */
    if (!relative_focused && event_last_user_time &&
        /* last user time must be strictly > launch_time to block focus */
        (event_time_after(event_last_user_time, launch_time) && event_last_user_time != launch_time) &&
        event_time_after(event_last_user_time, steal_time - OB_EVENT_USER_TIME_DELAY)) {
      steal = FALSE;
      ob_debug(
  ...
  if (!steal)
    ...
    ob_debug(
        "Allowing focus stealing for %s at time %u (last user "
        "interaction time %u)",
        self->title, steal_time, event_last_user_time);
  return steal;
}
...
static void client_restore_session_state(ObClient* self) {
  ...
  if (!(it = session_state_find(self))) {
    ob_debug_type(OB_DEBUG_SM, "Session data not found for client %s", self->title);
    return;
  }
  ...
  for (it = g_list_previous(mypos); it; it = g_list_previous(it)) {
    ...
    for (cit = client_list; cit; cit = g_list_next(cit)) {
      ...
      if (c->session == it->data) {
        stacking_below(CLIENT_AS_WINDOW(self), CLIENT_AS_WINDOW(cit->data));
        return TRUE;
      }
    }
  }
...
gboolean client_find_onscreen(ObClient* self, gint* x, gint* y, gint w, gint h, gboolean rude) {
  gint ox = *x, oy = *y;
...
static void client_get_startup_id(ObClient* self) {
  if (!(OBT_PROP_GETS_UTF8(self->window, NET_STARTUP_ID, &self->startup_id)))
    if (self->group)
      OBT_PROP_GETS_UTF8(self->group->leader, NET_STARTUP_ID, &self->startup_id);
}
...
static void client_get_desktop(ObClient* self) {
  ...
  if (OBT_PROP_GET32(self->window, NET_WM_DESKTOP, CARDINAL, &d)) {
    ...
    /* try get from the startup-notification protocol */
      ...
      if (self->desktop >= screen_num_desktops && self->desktop != DESKTOP_ALL)
        self->desktop = screen_num_desktops - 1;
      ob_debug("client desktop set from startup-notification: 0x%x", self->desktop);
    }
...
void client_get_mwm_hints(ObClient* self) {
  ...
  if (OBT_PROP_GETA32(self->window, MOTIF_WM_HINTS, MOTIF_WM_HINTS, &hints, &num)) {
...
void client_update_normal_hints(ObClient* self) {
  ...
  if (XGetWMNormalHints(obt_display, self->window, &size, &ret)) {
    ...
    */
    ...
    ob_debug("size inc (%d %d) base size (%d %d)", self->size_inc.width, self->size_inc.height, self->base_size.width,
...
static void client_setup_default_decor_and_functions(ObClient* self) {
  ...
  self->decorations = (OB_FRAME_DECOR_TITLEBAR | OB_FRAME_DECOR_HANDLE | OB_FRAME_DECOR_GRIPS | OB_FRAME_DECOR_BORDER |
                       OB_FRAME_DECOR_ICON | OB_FRAME_DECOR_ALLDESKTOPS | OB_FRAME_DECOR_ICONIFY |
                       OB_FRAME_DECOR_MAXIMIZE | OB_FRAME_DECOR_SHADE | OB_FRAME_DECOR_CLOSE);
  self->functions = (OB_CLIENT_FUNC_RESIZE | OB_CLIENT_FUNC_MOVE | OB_CLIENT_FUNC_ICONIFY | OB_CLIENT_FUNC_MAXIMIZE |
                     OB_CLIENT_FUNC_SHADE | OB_CLIENT_FUNC_CLOSE | OB_CLIENT_FUNC_BELOW | OB_CLIENT_FUNC_ABOVE |
                     OB_CLIENT_FUNC_UNDECORATE);

  if (!(self->min_size.width < self->max_size.width || self->min_size.height < self->max_size.height))
  ...
  switch (self->type) {
    ...
    case OB_CLIENT_TYPE_TOOLBAR:
      ...
      self->decorations &= ~(OB_FRAME_DECOR_ICONIFY | OB_FRAME_DECOR_MAXIMIZE);
  ...
  if (self->mwmhints.flags & OB_MWM_FLAG_DECORATIONS) {
    if (!(self->mwmhints.decorations & OB_MWM_DECOR_ALL)) {
      if (!((self->mwmhints.decorations & OB_MWM_DECOR_HANDLE) || (self->mwmhints.decorations & OB_MWM_DECOR_TITLE))) {
  ...
  if (!((self->functions & OB_CLIENT_FUNC_MAXIMIZE) && (self->functions & OB_CLIENT_FUNC_MOVE) &&
        (self->functions & OB_CLIENT_FUNC_RESIZE))) {
    self->functions &= ~OB_CLIENT_FUNC_MAXIMIZE;
...
void client_setup_decor_and_functions(ObClient* self, gboolean reconfig) {
  ...
  if (self->fullscreen) {
    self->functions &= (OB_CLIENT_FUNC_CLOSE | OB_CLIENT_FUNC_FULLSCREEN | OB_CLIENT_FUNC_ICONIFY);
    self->decorations = 0;
  }
...
static void client_change_allowed_actions(ObClient* self) {
  ...
  */
  ...
  if (!(self->functions & OB_CLIENT_FUNC_MAXIMIZE) && (self->max_horz || self->max_vert)) {
    if (self->frame)
...
void client_update_wmhints(ObClient* self) {
  ...
  if ((hints = XGetWMHints(obt_display, self->window)) != NULL) {
    ...
    if (hints->window_group != (self->group ? self->group->leader : None)) {
      ...
      */
      if (self->transient &&
          ((self->parents == NULL && oldgroup == NULL) || (self->transient_for_group && !self->group)))
        client_update_transient_for(self);
    }
...
void client_update_strut(ObClient* self) {
  ...
  if (OBT_PROP_GETA32(self->window, NET_WM_STRUT_PARTIAL, CARDINAL, &data, &num)) {
    if (num == 12) {
      got = TRUE;
      STRUT_PARTIAL_SET(strut, data[0], data[2], data[1], data[3], data[4], data[5], data[8], data[9], data[6], data[7],
                        data[10], data[11]);
    }
    g_free(data);
  }

  if (!got && OBT_PROP_GETA32(self->window, NET_WM_STRUT, CARDINAL, &data, &num)) {
    if (num == 4) {
      const Rect* a;
      ...
      STRUT_PARTIAL_SET(strut, data[0], data[2], data[1], data[3], a->y, a->y + a->height - 1, a->x,
                        a->x + a->width - 1, a->y, a->y + a->height - 1, a->x, a->x + a->width - 1);
    }
    g_free(data);
  }
...
void client_update_icons(ObClient* self) {
  ...
  if (OBT_PROP_GETA32(self->window, NET_WM_ICON, CARDINAL, &data, &num)) {
    ...
    while (i + 2 < num) { /* +2 is to make sure there is a w and h */
      ...
      for (j = 0; j < w * h; ++j)
        data[i + j] = ((guint)((data[i + j] >> 24) & 0xff) << RrDefaultAlphaOffset) +
                      ((guint)((data[i + j] >> 16) & 0xff) << RrDefaultRedOffset) +
                      ((guint)((data[i + j] >> 8) & 0xff) << RrDefaultGreenOffset) +
                      ((guint)((data[i + j] >> 0) & 0xff) << RrDefaultBlueOffset);
  ...
  if (!img) {
    ...
    if ((hints = XGetWMHints(obt_display, self->window))) {
      if (hints->flags & IconPixmapHint) {
        ...
        xicon = RrPixmapToRGBA(ob_rr_inst, hints->icon_pixmap, (hints->flags & IconMaskHint ? hints->icon_mask : None),
                               (gint*)&w, (gint*)&h, &data);
  ...
  */
  if (!self->icon_set && !self->parents) {
    ...
    ldata[1] = h;
    for (i = 0; i < w * h; ++i)
      ldata[i + 2] = ((guint)((icon[i] >> RrDefaultAlphaOffset) & 0xff) << 24) +
                     ((guint)((icon[i] >> RrDefaultRedOffset) & 0xff) << 16) +
                     ((guint)((icon[i] >> RrDefaultGreenOffset) & 0xff) << 8) +
                     ((guint)((icon[i] >> RrDefaultBlueOffset) & 0xff) << 0);
...
void client_update_icon_geometry(ObClient* self) {
  ...
  if (OBT_PROP_GETA32(self->window, NET_WM_ICON_GEOMETRY, CARDINAL, &data, &num)) {
    if (num == 4)
...
static void client_get_session_ids(ObClient* self) {
  ...
  if (self->group_name == NULL)
    self->group_name = g_strdup("");
  if (self->group_class == NULL)
    self->group_class = g_strdup("");
...
static void client_change_wm_state(ObClient* self) {
  ...
  if (self->shaded || self->iconic || (self->desktop != DESKTOP_ALL && self->desktop != screen_desktop)) {
...
ObClient* client_search_focus_tree_full(ObClient* self) {
  if (self->parents) {
    ...
    for (it = self->parents; it; it = g_slist_next(it)) {
      ObClient* c = it->data;
      if ((c = client_search_focus_tree_full(c)))
        return c;
    }
...
gboolean client_is_oldfullscreen(const ObClient* self, const Rect* area) {
  ...
  */
  ...
  if (self->decorations || self->max_horz || self->max_vert)
    return FALSE;
...
static ObStackingLayer calc_layer(ObClient* self) {
  ...
  else if ((self->fullscreen || client_is_oldfullscreen(self, &self->area)) &&
...
static void client_calc_layer_recursive(ObClient* self, ObClient* orig, ObStackingLayer min) {
...
gboolean client_enter_focusable(ObClient* self) {
  ...
  return (client_mouse_focusable(self) && self->type != OB_CLIENT_TYPE_DESKTOP);
}
...
static void client_apply_startup_state(ObClient* self, gint x, gint y, gint w, gint h) {
  ...
  /* turn them all off in the client, so they won't affect the window
     being placed */
  self->iconic = self->fullscreen = self->undecorated = self->shaded = self->demands_attention = self->max_horz =
      self->max_vert = FALSE;
  ...
  */
  ...
  ob_debug("placed window 0x%x at %d, %d with size %d x %d", self->window, x, y, w, h);
...
void client_try_configure(ObClient* self,
                          gint* x,
                          gint* y,
                          gint* w,
                          gint* h,
  ...
  *w = MIN(*w, (gint)G_MAXUSHORT - self->frame->size.left - self->frame->size.right);
  *h = MIN(*h, (gint)G_MAXUSHORT - self->frame->size.top - self->frame->size.bottom);
...
void client_configure(ObClient* self,
                      gint x,
                      gint y,
                      gint w,
                      gint h,
  ...
  if (send_resize_client && (w > oldclient.width || h > oldclient.height)) {
    XMoveResizeWindow(obt_display, self->window, self->frame->size.left, self->frame->size.top, MAX(w, oldclient.width),
  ...
  if (self->decorations != fdecor || self->max_horz != fhorz || self->max_vert != fvert) {
  ...
  */
  if ((!user && !resized && (rootmoved || force_reply)) ||
    ...
    ob_debug("Sending ConfigureNotify to %s for %d,%d %dx%d", self->title, self->root_pos.x, self->root_pos.y, w, h);
  ...
  if (send_resize_client && (w <= oldclient.width || h <= oldclient.height)) {
    frame_adjust_client_area(self->frame);
    XMoveResizeWindow(obt_display, self->window, self->frame->size.left, self->frame->size.top, w, h);
  }
  ...
  */
  if (self->managed &&
      (screen_find_monitor(&self->frame->area) != screen_find_monitor(&oldframe) ||
       (final && (client_is_oldfullscreen(self, &oldclient) != client_is_oldfullscreen(self, &self->area))))) {
    client_calc_layer(self);
  }
}
...
static void client_iconify_recursive(ObClient* self, gboolean iconic, gboolean curdesk, gboolean hide_animation) {
  GSList* it;
  gboolean changed = FALSE;

  if (self->iconic != iconic) {
    ob_debug("%sconifying window: 0x%lx", (iconic ? "I" : "Uni"), self->window);
    ...
    if (iconic) {
      ...
      if (curdesk && self->desktop != screen_desktop && self->desktop != DESKTOP_ALL)
  ...
  for (it = self->transients; it; it = g_slist_next(it))
    if (it->data != self)
      if (client_is_direct_child(self, it->data) || !iconic)
        client_iconify_recursive(it->data, iconic, curdesk, hide_animation);
}

void client_iconify(ObClient* self, gboolean iconic, gboolean curdesk, gboolean hide_animation) {
...
void client_maximize(ObClient* self, gboolean max, gint dir) {
  ...
  if (!(self->functions & OB_CLIENT_FUNC_MAXIMIZE) && max)
    return; /* can't */
...
void client_close(ObClient* self) {
  ...
  if (!self->delete_window)
    ...
    OBT_PROP_MSG_TO(self->window, self->window, WM_PROTOCOLS, OBT_PROP_ATOM(WM_DELETE_WINDOW), event_time(), 0, 0, 0,
                    NoEventMask);
...
static void client_prompt_kill(ObClient* self) {
  ...
  if (!self->kill_prompt) {
    ObPromptAnswer answers[] = {{0, OB_KILL_RESULT_NO}, {0, OB_KILL_RESULT_YES}};
    gchar* m;
    const gchar *y, *title;
    ...
    if (client_on_localhost(self)) {
      ...
      m = g_strdup_printf(_("The window \"%s\" does not seem to be responding.  Do you want to force it to exit by "
                            "sending the %s signal?"),
                          title, sig);
      y = _("End Process");
    }
    else {
    ...
    self->kill_prompt =
        prompt_new(m, NULL, answers, sizeof(answers) / sizeof(answers[0]), OB_KILL_RESULT_NO, /* default = no */
                   OB_KILL_RESULT_NO,                                                         /* cancel = no */
                   client_kill_requested, client_kill_cleanup, self);
    g_free(m);
  }
...
void client_kill(ObClient* self) {
  ...
  if (client_on_localhost(self) && self->pid) {
    ...
    if (self->kill_level == 0) {
      ob_debug("killing window 0x%x with pid %lu, with SIGTERM", self->window, self->pid);
      ...
      ob_debug("killing window 0x%x with pid %lu, with SIGKILL", self->window, self->pid);
...
void client_set_desktop(ObClient* self, guint target, gboolean donthide, gboolean dontraise) {
  self = client_search_top_direct_parent(self);
...
void client_set_state(ObClient* self, Atom action, glong data1, glong data2) {
  ...
  if (!(action == OBT_PROP_ATOM(NET_WM_STATE_ADD) || action == OBT_PROP_ATOM(NET_WM_STATE_REMOVE) ||
  ...
  for (i = 0; i < 2; ++i) {
    ...
    if (action == OBT_PROP_ATOM(NET_WM_STATE_TOGGLE)) {
      ...
      action = value ? OBT_PROP_ATOM(NET_WM_STATE_REMOVE) : OBT_PROP_ATOM(NET_WM_STATE_ADD);
    }
...
gboolean client_focus(ObClient* self) {
  ...
  ob_debug_type(OB_DEBUG_FOCUS, "Focusing client \"%s\" (0x%x) at time %u", self->title, self->window, event_time());
  ...
  if (self->can_focus) {
    ...
    XSetInputFocus(obt_display, self->window, RevertToPointerRoot, event_time());
  }
  ...
  ob_debug_type(OB_DEBUG_FOCUS, "Error focusing? %d", obt_display_error_occured);
...
void client_activate(ObClient* self, gboolean desktop, gboolean here, gboolean raise, gboolean unshade, gboolean user) {
  self = client_focus_target(self);
...
static void client_bring_windows_recursive(ObClient* self,
  ...
  for (it = self->transients; it; it = g_slist_next(it))
    client_bring_windows_recursive(it->data, desktop, helpers, modals, iconic);

  if (((helpers && client_helper(self)) || (modals && self->modal)) &&
      (!screen_compare_desktops(self->desktop, desktop) || (iconic && self->iconic))) {
    if (iconic && self->iconic)
      client_iconify(self, FALSE, TRUE, FALSE);
    else
...
static void detect_edge(Rect area,
                        ObDirection dir,
                        gint my_head,
                        gint my_size,
                        gint my_edge_start,
...
void client_find_edge_directional(ObClient* self,
                                  ObDirection dir,
                                  gint my_head,
                                  gint my_size,
                                  gint my_edge_start,
  ...
  *near_edge = TRUE;
  ...
  detect_edge(dock_area, dir, my_head, my_size, my_edge_start, my_edge_size, dest, near_edge);

  g_slice_free(Rect, a);
}

void client_find_move_directional(ObClient* self, ObDirection dir, gint* x, gint* y) {
  gint head, size;
  ...
  client_find_edge_directional(self, dir, head, size, e_start, e_size, &e, &near);
  *x = self->frame->area.x;
  *y = self->frame->area.y;
  switch (dir) {
    case OB_DIRECTION_EAST:
    ...
    for (it = stacking_list; it; it = g_list_next(it)) {
      if (WINDOW_IS_CLIENT(it->data)) {
        ...
        if (c->frame->visible &&
            ...
            !frame_iconify_animating(c->frame) && RECT_CONTAINS(c->frame->area, x, y)) {
          ret = c;
          break;
        }
      }
...
gboolean client_has_relative(ObClient* self) {
  return client_has_parent(self) || client_has_group_siblings(self) || client_has_children(self);
}
