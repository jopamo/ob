...
static gboolean self_update(ObMenuFrame* frame, gpointer data) {
  ...
  for (desktop = 0; desktop < screen_num_desktops; desktop++) {
    ...
    for (it = focus_order; it; it = g_list_next(it)) {
      ObClient* c = it->data;
      if (focus_valid_target(c, desktop, TRUE, TRUE, FALSE, TRUE, FALSE, FALSE, FALSE)) {
        empty = FALSE;
...
static void menu_execute(ObMenuEntry* self, ObMenuFrame* f, ObClient* c, guint state, gpointer data) {
