...
ObPrompt* prompt_new(const gchar* msg,
  ...
  self->super.window = XCreateWindow(obt_display, obt_root(ob_screen), 0, 0, 1, 1, 0, CopyFromParent, InputOutput,
                                     CopyFromParent, CWOverrideRedirect, &attrib);
  self->ic = obt_keyboard_context_new(self->super.window, self->super.window);

  /* make it a dialog type window */
  OBT_PROP_SET32(self->super.window, NET_WM_WINDOW_TYPE, ATOM, OBT_PROP_ATOM(NET_WM_WINDOW_TYPE_DIALOG));
  ...
  self->msg.window = XCreateWindow(obt_display, self->super.window, 0, 0, 1, 1, 0, CopyFromParent, InputOutput,
  ...
  for (i = 0; i < self->n_buttons; ++i) {
    self->button[i].window = XCreateWindow(obt_display, self->super.window, 0, 0, 1, 1, 0, CopyFromParent, InputOutput,
                                           CopyFromParent, 0, NULL);
    ...
    XSelectInput(obt_display, self->button[i].window, ButtonPressMask | ButtonReleaseMask | ButtonMotionMask);
  }
...
static void prompt_layout(ObPrompt* self) {
  ...
  XMoveResizeWindow(obt_display, self->msg.window, self->msg.x, self->msg.y, self->msg.width, self->msg.height);
  for (i = 0; i < self->n_buttons; ++i)
    XMoveResizeWindow(obt_display, self->button[i].window, self->button[i].x, self->button[i].y, self->button[i].width,
                      self->button[i].height);
}
...
static void prompt_resize(ObPrompt* self, gint w, gint h) {
  ...
  if (self->mapped) {
    ...
    XSendEvent(req.display, req.window, FALSE, StructureNotifyMask, (XEvent*)&req);
  }
  else
    XResizeWindow(obt_display, self->super.window, w, h);
}

static void setup_button_focus_tex(ObPromptElement* e, RrAppearance* a, gboolean on) {
  gint i, l, r, t, b;
...
void prompt_show(ObPrompt* self, ObClient* parent, gboolean modal) {
  ...
  if (self->mapped) {
    /* activate the prompt */
    OBT_PROP_MSG(ob_screen, self->super.window, NET_ACTIVE_WINDOW, 1, /* from an application.. */
                 event_time(), 0, 0, 0);
    return;
  }
  ...
  if (parent) {
    ...
    nstates = (modal ? 1 : 0);
    OBT_PROP_SETA32(self->super.window, NET_WM_STATE, ATOM, states, nstates);
  }
  else
...
gboolean prompt_key_event(ObPrompt* self, XEvent* e) {
  ...
  if (sym == XK_Escape)
    ...
    for (i = 0; i < self->n_buttons; ++i)
      if (self->focus == &self->button[i])
        break;
    i += (left ? -1 : 1);
    if (i < 0)
      i = self->n_buttons - 1;
    else if (i >= self->n_buttons)
      i = 0;
    self->focus = &self->button[i];

    if (oldfocus != self->focus)
      render_button(self, oldfocus);
...
gboolean prompt_mouse_event(ObPrompt* self, XEvent* e) {
  ...
  but = NULL;
  for (i = 0; i < self->n_buttons; ++i)
    if (self->button[i].window == (e->type == MotionNotify ? e->xmotion.window : e->xbutton.window)) {
      but = &self->button[i];
      break;
    }
  if (!but)
  ...
  if (e->type == ButtonPress) {
    ...
    but->pressed = FALSE;
    if (but->hover) {
      /* prompt_run_callback may free the prompt, so avoid touching it after */
    ...
    if (but->pressed) {
      ...
      hover = (e->xmotion.x >= 0 && e->xmotion.y >= 0 && e->xmotion.x < but->width && e->xmotion.y < but->height);
...
ObPrompt* prompt_show_message(const gchar* msg, const gchar* title, const gchar* answer) {
  ObPrompt* p;
  ObPromptAnswer ans[] = {{answer, 0}};

  p = prompt_new(msg, title, ans, 1, 0, 0, prompt_show_message_cb, prompt_show_message_cleanup, NULL);
