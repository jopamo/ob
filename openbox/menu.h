...
typedef gboolean (*ObMenuUpdateFunc)(struct _ObMenuFrame* frame, gpointer data);
...
/*! @param x is the mouse x coordinate. on return it should be the x coordinate
    ...
    @param y is the mouse y coordinate. on return it should be the y coordinate
             for the menu
*/
typedef void (*ObMenuPlaceFunc)(struct _ObMenuFrame* frame, gint* x, gint* y, gboolean mouse, gpointer data);
...
typedef enum { OB_MENU_ENTRY_TYPE_NORMAL, OB_MENU_ENTRY_TYPE_SUBMENU, OB_MENU_ENTRY_TYPE_SEPARATOR } ObMenuEntryType;
...
ObMenuEntry* menu_add_normal(ObMenu* menu, gint id, const gchar* label, GSList* actions, gboolean allow_shortcut);
