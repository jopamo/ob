...
static void parse_single_per_app_settings(xmlNodePtr app, ObAppSettings* settings) {
  xmlNodePtr n, c;
  ...
  if ((n = obt_xml_find_node(app->children, "size"))) {
    if ((c = obt_xml_find_node(n->children, "width"))) {
      if (!obt_xml_node_contains(c, "default")) {
        gchar* s = obt_xml_node_string(c);
        config_parse_relative_number(s, &settings->width_num, &settings->width_denom);
    ...
    if ((c = obt_xml_find_node(n->children, "height"))) {
      if (!obt_xml_node_contains(c, "default")) {
        gchar* s = obt_xml_node_string(c);
        config_parse_relative_number(s, &settings->height_num, &settings->height_denom);
  ...
  if ((n = obt_xml_find_node(app->children, "maximized"))) {
    if (!obt_xml_node_contains(n, "default")) {
      ...
      if (!g_ascii_strcasecmp(s, "horizontal")) {
        ...
        settings->max_horz = settings->max_vert = obt_xml_node_bool(n);
      g_free(s);
    }
  }
}
...
static void parse_per_app_settings(xmlNodePtr node, gpointer d) {
  ...
  for (; app; app = obt_xml_find_node(app->next, "application")) {
    ...
    gboolean name_set, class_set, role_set, title_set, type_set, group_name_set, group_class_set;
    gchar *name = NULL, *class = NULL, *role = NULL, *title = NULL, *type_str = NULL, *group_name = NULL,
          *group_class = NULL;
    ObClientType type;
    ...
    if (!(class_set || name_set || role_set || title_set || type_set || group_class_set || group_name_set))
      continue;
...
static void parse_mouse(xmlNodePtr node, gpointer d) {
  ...
  for (n = obt_xml_find_node(node, "context"); n; n = obt_xml_find_node(n->next, "context")) {
    gchar* modcxstr;
    ObFrameContext cx;
    ...
    while (frame_next_context_from_string(modcxstr, &cx)) {
      ...
      for (nbut = obt_xml_find_node(n->children, "mousebind"); nbut;
        ...
        for (nact = obt_xml_find_node(nbut->children, "action"); nact; nact = obt_xml_find_node(nact->next, "action")) {
          ObActionsAct* action;
...
static void parse_theme(xmlNodePtr node, gpointer d) {
  ...
  for (n = obt_xml_find_node(node, "font"); n; n = obt_xml_find_node(n->next, "font")) {
    xmlNodePtr fnode;
    RrFont** font;
...
static void parse_desktops(xmlNodePtr node, gpointer d) {
  ...
  if ((n = obt_xml_find_node(node, "names"))) {
    ...
    for (nname = obt_xml_find_node(n->children, "name"); nname; nname = obt_xml_find_node(nname->next, "name")) {
      config_desktops_names = g_slist_append(config_desktops_names, obt_xml_node_string(nname));
    }
  }
    ...
    if (obt_xml_node_contains(n, "Top"))
      ...
      if ((n = obt_xml_find_node(node, "popupFixedPosition"))) {
        ...
        if ((n2 = obt_xml_find_node(n->children, "x")))
          config_parse_gravity_coord(n2, &config_resize_popup_fixed.x);
        if ((n2 = obt_xml_find_node(n->children, "y")))
          config_parse_gravity_coord(n2, &config_resize_popup_fixed.y);

        config_resize_popup_fixed.x.pos = MAX(config_resize_popup_fixed.x.pos, 0);
        config_resize_popup_fixed.y.pos = MAX(config_resize_popup_fixed.y.pos, 0);
      }
    }
  }
}
...
static void parse_dock(xmlNodePtr node, gpointer d) {
  ...
  if ((n = obt_xml_find_node(node, "position"))) {
    if (obt_xml_node_contains(n, "TopLeft"))
      config_dock_floating = FALSE, config_dock_pos = OB_DIRECTION_NORTHWEST;
    else if (obt_xml_node_contains(n, "Top"))
      config_dock_floating = FALSE, config_dock_pos = OB_DIRECTION_NORTH;
    else if (obt_xml_node_contains(n, "TopRight"))
      config_dock_floating = FALSE, config_dock_pos = OB_DIRECTION_NORTHEAST;
    else if (obt_xml_node_contains(n, "Right"))
      config_dock_floating = FALSE, config_dock_pos = OB_DIRECTION_EAST;
    else if (obt_xml_node_contains(n, "BottomRight"))
      config_dock_floating = FALSE, config_dock_pos = OB_DIRECTION_SOUTHEAST;
    else if (obt_xml_node_contains(n, "Bottom"))
      config_dock_floating = FALSE, config_dock_pos = OB_DIRECTION_SOUTH;
    else if (obt_xml_node_contains(n, "BottomLeft"))
      config_dock_floating = FALSE, config_dock_pos = OB_DIRECTION_SOUTHWEST;
    else if (obt_xml_node_contains(n, "Left"))
      config_dock_floating = FALSE, config_dock_pos = OB_DIRECTION_WEST;
...
static void parse_menu(xmlNodePtr node, gpointer d) {
  ...
  for (node = obt_xml_find_node(node, "file"); node; node = obt_xml_find_node(node->next, "file")) {
    gchar* c = obt_xml_node_string(node);
    config_menu_files = g_slist_append(config_menu_files, obt_paths_expand_tilde(c));
    g_free(c);
  }
}
...
static void bind_default_mouse(void) {
  ObDefMouseBind* it;
  ObDefMouseBind binds[] = {{"Left", "Client", OB_MOUSE_ACTION_PRESS, "Focus"},
