...
#define FRAME_EVENTMASK (ButtonPressMask | ButtonMotionMask | EnterWindowMask | LeaveWindowMask)
#define ENTRY_EVENTMASK (EnterWindowMask | LeaveWindowMask | ButtonPressMask | ButtonReleaseMask | PointerMotionMask)
...
static ObMenuEntryFrame* menu_entry_frame_new(ObMenuEntry* entry, ObMenuFrame* frame);
...
static Window createWindow(Window parent, gulong mask, XSetWindowAttributes* attrib) {
...
void menu_frame_startup(gboolean reconfig) {
  ...
  for (i = 0; i < ob_rr_theme->menu_sep_width; ++i) {
    ...
    a_sep->texture[i].data.lineart.color = ob_rr_theme->menu_sep_color;
  }
...
ObMenuFrame* menu_frame_new(ObMenu* menu, guint show_from, ObClient* client) {
  ...
  self->window = createWindow(obt_root(ob_screen), CWEventMask, &attr);
...
static ObMenuEntryFrame* menu_entry_frame_new(ObMenuEntry* entry, ObMenuFrame* frame) {
  ...
  if ((entry->type == OB_MENU_ENTRY_TYPE_NORMAL) || (entry->type == OB_MENU_ENTRY_TYPE_SUBMENU)) {
...
static void menu_entry_frame_free(ObMenuEntryFrame* self) {
  if (self) {
    ...
    if ((self->entry->type == OB_MENU_ENTRY_TYPE_NORMAL) || (self->entry->type == OB_MENU_ENTRY_TYPE_SUBMENU)) {
...
static void menu_frame_place_topmenu(ObMenuFrame* self,
  ...
  screen_apply_gravity_point(x, y, self->area.width, self->area.height, pos, screen_physical_area_monitor(monitor));
...
static void menu_frame_place_submenu(ObMenuFrame* self, gint* x, gint* y) {
  ...
  if (self->direction_right)
    *x = self->parent->area.x + self->parent->area.width - overlapx - bwidth;
  else
...
static void menu_entry_frame_render(ObMenuEntryFrame* self) {
  ...
  switch (self->entry->type) {
    ...
    case OB_MENU_ENTRY_TYPE_SUBMENU:
      item_a =
          (self->entry->type == OB_MENU_ENTRY_TYPE_NORMAL && !self->entry->data.normal.enabled ?
                                                                                               /* disabled */
               (self == self->frame->selected ? ob_rr_theme->a_menu_disabled_selected : ob_rr_theme->a_menu_disabled)
                                                                                               :
                                                                                               /* enabled */
               (self == self->frame->selected ? ob_rr_theme->a_menu_selected : ob_rr_theme->a_menu_normal));
      th = ITEM_HEIGHT;
      break;
    case OB_MENU_ENTRY_TYPE_SEPARATOR:
      if (self->entry->data.separator.label) {
        ...
        th = ob_rr_theme->menu_sep_width + 2 * ob_rr_theme->menu_sep_paddingy;
      }
      break;
    default:
  ...
  switch (self->entry->type) {
    case OB_MENU_ENTRY_TYPE_NORMAL:
      text_a =
          (self->entry->type == OB_MENU_ENTRY_TYPE_NORMAL && !self->entry->data.normal.enabled ?
                                                                                               /* disabled */
               (self == self->frame->selected ? ob_rr_theme->a_menu_text_disabled_selected
                                              : ob_rr_theme->a_menu_text_disabled)
                                                                                               :
                                                                                               /* enabled */
               (self == self->frame->selected ? ob_rr_theme->a_menu_text_selected : ob_rr_theme->a_menu_text_normal));
      text_a->texture[0].data.text.string = self->entry->data.normal.label;
      if (self->entry->data.normal.shortcut &&
          (self->frame->menu->show_all_shortcuts || self->entry->data.normal.shortcut_always_show ||
           self->entry->data.normal.shortcut_position > 0)) {
        text_a->texture[0].data.text.shortcut = TRUE;
        text_a->texture[0].data.text.shortcut_pos = self->entry->data.normal.shortcut_position;
      }
      else
        text_a->texture[0].data.text.shortcut = FALSE;
      break;
    case OB_MENU_ENTRY_TYPE_SUBMENU:
      text_a = (self == self->frame->selected ? ob_rr_theme->a_menu_text_selected : ob_rr_theme->a_menu_text_normal);
      sub = self->entry->data.submenu.submenu;
      text_a->texture[0].data.text.string = sub ? sub->title : "";
      if (sub && sub->shortcut &&
          (self->frame->menu->show_all_shortcuts || sub->shortcut_always_show || sub->shortcut_position > 0)) {
        text_a->texture[0].data.text.shortcut = TRUE;
    ...
    case OB_MENU_ENTRY_TYPE_SEPARATOR:
      if (self->entry->data.separator.label != NULL) {
        ...
        text_a->texture[0].data.text.string = self->entry->data.separator.label;
      }
      else
  ...
  switch (self->entry->type) {
    case OB_MENU_ENTRY_TYPE_NORMAL:
      XMoveResizeWindow(obt_display, self->text, self->frame->text_x, PADDING, self->frame->text_w,
                        ITEM_HEIGHT - 2 * PADDING);
      text_a->surface.parent = item_a;
      text_a->surface.parentx = self->frame->text_x;
      text_a->surface.parenty = PADDING;
      RrPaint(text_a, self->text, self->frame->text_w, ITEM_HEIGHT - 2 * PADDING);
      break;
    case OB_MENU_ENTRY_TYPE_SUBMENU:
      XMoveResizeWindow(obt_display, self->text, self->frame->text_x, PADDING, self->frame->text_w - ITEM_HEIGHT,
                        ITEM_HEIGHT - 2 * PADDING);
      text_a->surface.parent = item_a;
      text_a->surface.parentx = self->frame->text_x;
      text_a->surface.parenty = PADDING;
      RrPaint(text_a, self->text, self->frame->text_w - ITEM_HEIGHT, ITEM_HEIGHT - 2 * PADDING);
      break;
    case OB_MENU_ENTRY_TYPE_SEPARATOR:
      if (self->entry->data.separator.label != NULL) {
        /* labeled separator */
        XMoveResizeWindow(obt_display, self->text, ob_rr_theme->paddingx, ob_rr_theme->paddingy,
                          self->area.width - 2 * ob_rr_theme->paddingx,
                          ob_rr_theme->menu_title_height - 2 * ob_rr_theme->paddingy);
        text_a->surface.parent = item_a;
        text_a->surface.parentx = ob_rr_theme->paddingx;
        text_a->surface.parenty = ob_rr_theme->paddingy;
        RrPaint(text_a, self->text, self->area.width - 2 * ob_rr_theme->paddingx,
                ob_rr_theme->menu_title_height - 2 * ob_rr_theme->paddingy);
      }
      else {
        gint i;
        ...
        /* unlabeled separator */
        XMoveResizeWindow(obt_display, self->text, 0, 0, self->area.width,
                          ob_rr_theme->menu_sep_width + 2 * ob_rr_theme->menu_sep_paddingy);

        a_sep->surface.parent = item_a;
        a_sep->surface.parentx = 0;
        a_sep->surface.parenty = 0;
        for (i = 0; i < ob_rr_theme->menu_sep_width; ++i) {
          a_sep->texture[i].data.lineart.x1 = ob_rr_theme->menu_sep_paddingx;
          a_sep->texture[i].data.lineart.y1 = ob_rr_theme->menu_sep_paddingy + i;
          a_sep->texture[i].data.lineart.x2 = self->area.width - ob_rr_theme->menu_sep_paddingx - 1;
          a_sep->texture[i].data.lineart.y2 = ob_rr_theme->menu_sep_paddingy + i;
        }

        RrPaint(a_sep, self->text, self->area.width, ob_rr_theme->menu_sep_width + 2 * ob_rr_theme->menu_sep_paddingy);
      }
      break;
    default:
      g_assert_not_reached();
  ...
  if (((self->entry->type == OB_MENU_ENTRY_TYPE_NORMAL) || (self->entry->type == OB_MENU_ENTRY_TYPE_SUBMENU)) &&
    ...
    XMoveResizeWindow(obt_display, self->icon, PADDING, frame->item_margin.top,
                      ITEM_HEIGHT - frame->item_margin.top - frame->item_margin.bottom,
                      ITEM_HEIGHT - frame->item_margin.top - frame->item_margin.bottom);

    clear = ob_rr_theme->a_clear_tex;
    RrAppearanceClearTextures(clear);
    clear->texture[0].type = RR_TEXTURE_IMAGE;
    clear->texture[0].data.image.image = self->entry->data.normal.icon;
    clear->texture[0].data.image.alpha = self->entry->data.normal.icon_alpha;
    clear->surface.parent = item_a;
    clear->surface.parentx = PADDING;
    clear->surface.parenty = frame->item_margin.top;
    RrPaint(clear, self->icon, ITEM_HEIGHT - frame->item_margin.top - frame->item_margin.bottom,
            ITEM_HEIGHT - frame->item_margin.top - frame->item_margin.bottom);
    XMapWindow(obt_display, self->icon);
  }
  else if (self->entry->type == OB_MENU_ENTRY_TYPE_NORMAL && self->entry->data.normal.mask) {
    RrColor* c;
    RrAppearance* clear;
    ...
    clear->texture[0].data.mask.mask = self->entry->data.normal.mask;

    c = (self->entry->type == OB_MENU_ENTRY_TYPE_NORMAL && !self->entry->data.normal.enabled ?
                                                                                             /* disabled */
             (self == self->frame->selected ? self->entry->data.normal.mask_disabled_selected_color
  ...
  if (self->entry->type == OB_MENU_ENTRY_TYPE_SUBMENU) {
    ...
    XMoveResizeWindow(obt_display, self->bullet, self->frame->text_x + self->frame->text_w - ITEM_HEIGHT + PADDING,
                      PADDING, ITEM_HEIGHT - 2 * PADDING, ITEM_HEIGHT - 2 * PADDING);
    bullet_a =
        (self == self->frame->selected ? ob_rr_theme->a_menu_bullet_selected : ob_rr_theme->a_menu_bullet_normal);
    bullet_a->surface.parent = item_a;
    bullet_a->surface.parentx = self->frame->text_x + self->frame->text_w - ITEM_HEIGHT + PADDING;
    bullet_a->surface.parenty = PADDING;
    RrPaint(bullet_a, self->bullet, ITEM_HEIGHT - 2 * PADDING, ITEM_HEIGHT - 2 * PADDING);
...
/*! this code is taken from the menu_frame_render. if that changes, this won't
  work.. */
static gint menu_entry_frame_get_height(ObMenuEntryFrame* self, gboolean first_entry, gboolean last_entry) {
  ObMenuEntryType t;
  gint h = 0;
  ...
  switch (t) {
    ...
    case OB_MENU_ENTRY_TYPE_SEPARATOR:
      if (self->entry->data.separator.label != NULL) {
        h += ob_rr_theme->menu_title_height + (ob_rr_theme->mbwidth - PADDING) * 2;
        ...
        h += ob_rr_theme->menu_sep_width + 2 * ob_rr_theme->menu_sep_paddingy - PADDING * 2;
      }
      break;
  }
...
void menu_frame_render(ObMenuFrame* self) {
  ...
  if (self->entries) {
    ...
    STRUT_SET(self->item_margin, MAX(self->item_margin.left, l), MAX(self->item_margin.top, t),
              MAX(self->item_margin.right, r), MAX(self->item_margin.bottom, b));
  ...
  for (it = self->entries; it; it = g_list_next(it)) {
    ...
    /* if the first entry is a labeled separator, then make its border
       overlap with the menu's outside border */
    if (it == self->entries && e->entry->type == OB_MENU_ENTRY_TYPE_SEPARATOR && e->entry->data.separator.label) {
      h -= ob_rr_theme->mbwidth;
    }

    if (e->entry->type == OB_MENU_ENTRY_TYPE_SEPARATOR && e->entry->data.separator.label) {
    ...
    text_a = (e->entry->type == OB_MENU_ENTRY_TYPE_NORMAL && !e->entry->data.normal.enabled ?
                                                                                            /* disabled */
                  (e == self->selected ? ob_rr_theme->a_menu_text_disabled_selected : ob_rr_theme->a_menu_text_disabled)
                                                                                            :
                                                                                            /* enabled */
                  (e == self->selected ? ob_rr_theme->a_menu_text_selected : ob_rr_theme->a_menu_text_normal));
    switch (e->entry->type) {
      case OB_MENU_ENTRY_TYPE_NORMAL:
        ...
        if (e->entry->data.normal.icon || e->entry->data.normal.mask)
          has_icon = TRUE;
        break;
      case OB_MENU_ENTRY_TYPE_SUBMENU:
      ...
      case OB_MENU_ENTRY_TYPE_SEPARATOR:
        if (e->entry->data.separator.label != NULL) {
          ob_rr_theme->a_menu_text_title->texture[0].data.text.string = e->entry->data.separator.label;
          tw = RrMinWidth(ob_rr_theme->a_menu_text_title) + 2 * ob_rr_theme->paddingx;
          tw = MIN(tw, MAX_MENU_WIDTH);
          th = ob_rr_theme->menu_title_height + (ob_rr_theme->mbwidth - PADDING) * 2;
        }
        else {
          tw = 0;
          th = ob_rr_theme->menu_sep_width + 2 * ob_rr_theme->menu_sep_paddingy - 2 * PADDING;
        }
        break;
      default:
        g_assert_not_reached();
  ...
  e = it ? it->data : NULL;
  if (e && e->entry->type == OB_MENU_ENTRY_TYPE_SEPARATOR && e->entry->data.separator.label) {
    h -= ob_rr_theme->mbwidth;
  }
...
static void menu_frame_update(ObMenuFrame* self) {
  ...
  /* go through the menu's and frame's entries and connect the frame entries
     to the menu entries */
  for (fit = self->entries; mit && fit; mit = g_list_next(mit), fit = g_list_next(fit)) {
  ...
  for (fit = self->entries; fit; fit = g_list_next(fit))
    h += menu_entry_frame_get_height(fit->data, fit == self->entries, g_list_next(fit) == NULL);
  ...
  if (h > a->height) {
...

    {
      ...
      */
      more_entry = menu_get_more((self->menu->more_menu ? self->menu->more_menu : self->menu),
                                 /* continue where we left off */
                                 self->show_from + g_list_length(self->entries));
...
gboolean menu_frame_show_topmenu(ObMenuFrame* self,
                                 const GravityPoint* pos,
                                 gint monitor,
                                 gboolean mouse,
                                 gboolean user_positioned) {
  ...
  if (self->menu->place_func) {
    ...
    menu_frame_place_topmenu(self, pos, &x, &y, monitor, user_positioned);
  }
...
void menu_frame_select(ObMenuFrame* self, ObMenuEntryFrame* entry, gboolean immediate) {
  ...
  if (old == entry && (!old || old->entry->type != OB_MENU_ENTRY_TYPE_SUBMENU))
    return;
  ...
  if (oldchild_entry) {
    ...
    if (oldchild_entry == self->selected) {
      ...
      if (immediate || config_submenu_hide_delay == 0)
        ...
        if (submenu_hide_timer)
          g_source_remove(submenu_hide_timer);
        submenu_hide_timer = g_timeout_add_full(G_PRIORITY_DEFAULT, config_submenu_hide_delay, submenu_hide_timeout,
                                                oldchild, submenu_hide_dest);
      }
    }
  }
      ...
      if (oldchild_entry != self->selected) {
        if (immediate || config_submenu_show_delay == 0)
          ...
          if (submenu_show_timer)
            g_source_remove(submenu_show_timer);
          submenu_show_timer = g_timeout_add_full(G_PRIORITY_DEFAULT, config_submenu_show_delay, submenu_show_timeout,
...
void menu_entry_frame_show_submenu(ObMenuEntryFrame* self) {
  ...
  if (!self->entry->data.submenu.submenu)
    return;

  f = menu_frame_new(self->entry->data.submenu.submenu, self->entry->data.submenu.show_from, self->frame->client);
...
void menu_entry_frame_execute(ObMenuEntryFrame* self, guint state) {
  if (self->entry->type == OB_MENU_ENTRY_TYPE_NORMAL && self->entry->data.normal.enabled) {
    ...
    if (func)
      ...
      actions_run_acts(acts, OB_USER_ACTION_MENU_SELECTION, state, -1, -1, 0, OB_FRAME_CONTEXT_NONE, client);
  }
}
