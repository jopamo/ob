...
static void prompt_handler(const gchar* log_domain, GLogLevelFlags log_level, const gchar* message, gpointer user_data);

void ob_debug_startup(void) {
  ObtPaths* p = obt_paths_new();
  gchar* dir = g_build_filename(obt_paths_cache_home(p), "openbox", NULL);
  ...
  if (!obt_paths_mkdir_path(dir, 0777))
    ...
    gchar* name = g_build_filename(obt_paths_cache_home(p), "openbox", "openbox.log", NULL);
  ...
  rr_handler_id =
      g_log_set_handler("ObRender", G_LOG_LEVEL_MASK | G_LOG_FLAG_FATAL | G_LOG_FLAG_RECURSION, log_handler, NULL);
  obt_handler_id =
      g_log_set_handler("Obt", G_LOG_LEVEL_MASK | G_LOG_FLAG_FATAL | G_LOG_FLAG_RECURSION, log_handler, NULL);
  ob_handler_id =
      g_log_set_handler("Openbox", G_LOG_LEVEL_MASK | G_LOG_FLAG_FATAL | G_LOG_FLAG_RECURSION, log_handler, NULL);
  ob_handler_prompt_id = g_log_set_handler("Openbox", G_LOG_LEVEL_MASK & ~G_LOG_LEVEL_DEBUG, prompt_handler, NULL);
...
static void log_handler(const gchar* log_domain, GLogLevelFlags log_level, const gchar* message, gpointer data) {
  ...
  switch (log_level & G_LOG_LEVEL_MASK) {
    ...
    case G_LOG_LEVEL_ERROR:
      ...
      break;
    default:
      g_assert_not_reached(); /* invalid level.. */
  }
...
static void prompt_handler(const gchar* log_domain, GLogLevelFlags log_level, const gchar* message, gpointer data) {
...
static inline void log_argv(ObDebugType type, const gchar* format, va_list args) {
  const gchar* prefix;
  gchar* message;
