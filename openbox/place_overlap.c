...
static int total_overlap(const Rect* client_rects, int n_client_rects, const Rect* proposed_rect);
...
void place_overlap_find_least_placement(const Rect* client_rects,
  ...
  make_grid(client_rects, n_client_rects, monitor, x_edges, y_edges, max_edges);
  int i;
  for (i = 0; i < max_edges; ++i) {
    ...
    for (j = 0; j < max_edges; ++j) {
      ...
      Point best_top_left;
      int this_overlap = best_direction(&grid_point, client_rects, n_client_rects, monitor, req_size, &best_top_left);
  ...
  if (config_place_center && overlap == 0) {
    center_in_field(result, req_size, monitor, client_rects, n_client_rects, x_edges, y_edges, max_edges);
  }
}
...
static int find_first_grid_position_greater_or_equal(int search_value, const int* edges, int max_edges) {
...
static int expand_field(int orig_edge_index, const int* edges, ExpandByMethod expand_by, const ExpandInfo* i) {
  Rect field;
  RECT_SET(field, i->top_left->x, i->top_left->y, i->orig_width, i->orig_height);
...
static void center_in_field(Point* top_left,
  ...
  int orig_right_edge_index =
      find_first_grid_position_greater_or_equal(top_left->x + req_size->width, x_edges, max_edges);
  int orig_bottom_edge_index =
      find_first_grid_position_greater_or_equal(top_left->y + req_size->height, y_edges, max_edges);
  ExpandInfo i = {.top_left = top_left,
                  ...
                  .max_edges = max_edges};
  /* Try extending width. */
  int right_edge_index = expand_field(orig_right_edge_index, x_edges, expand_width, &i);
  /* Try extending height. */
  int bottom_edge_index = expand_field(orig_bottom_edge_index, y_edges, expand_height, &i);

  int final_width = x_edges[orig_right_edge_index] - top_left->x;
  int final_height = y_edges[orig_bottom_edge_index] - top_left->y;
  if (right_edge_index == orig_right_edge_index && bottom_edge_index != orig_bottom_edge_index)
    final_height = y_edges[bottom_edge_index] - top_left->y;
  else if (right_edge_index != orig_right_edge_index && bottom_edge_index == orig_bottom_edge_index)
...
static int best_direction(const Point* grid_point,
                          ...
                          const Size* req_size,
                          Point* best_top_left) {
  static const Size directions[NUM_DIRECTIONS] = {{0, 0}, {0, -1}, {-1, 0}, {-1, -1}};
  int overlap = G_MAXINT;
  int i;
  for (i = 0; i < NUM_DIRECTIONS; ++i) {
    Point pt = {.x = grid_point->x + (req_size->width * directions[i].width),
