...
static void dock_app_grab_button(ObDockApp* app, gboolean grab) {
  if (grab) {
    grab_button_full(config_dock_app_move_button, config_dock_app_move_modifiers, app->icon_win,
                     ButtonPressMask | ButtonReleaseMask | ButtonMotionMask, GrabModeAsync, OB_CURSOR_MOVE);
  }
  else {
    ungrab_button(config_dock_app_move_button, config_dock_app_move_modifiers, app->icon_win);
  }
}
...
void dock_startup(gboolean reconfig) {
  ...
  dock->dock_map = g_hash_table_new((GHashFunc)window_hash, (GEqualFunc)window_comp);
...
void dock_manage(Window icon_win, Window name_win) {
  ...
  */
  ...
  ob_debug("Managed Dock App: 0x%lx 0x%lx (%s)", app->icon_win, app->name_win, app->class);
...
void dock_configure(void) {
  ...
  if (!dock->dock_apps) {
    ...
    switch (config_dock_pos) {
      case OB_DIRECTION_NORTHWEST:
        switch (config_dock_orient) {
          case OB_ORIENTATION_HORZ:
            STRUT_PARTIAL_SET(dock_strut, 0, strh, 0, 0, 0, 0, dock->area.x, dock->area.x + dock->area.width - 1, 0, 0,
                              0, 0);
            break;
          case OB_ORIENTATION_VERT:
            STRUT_PARTIAL_SET(dock_strut, strw, 0, 0, 0, dock->area.y, dock->area.y + dock->area.height - 1, 0, 0, 0, 0,
                              0, 0);
            break;
        }
        break;
      case OB_DIRECTION_NORTH:
        STRUT_PARTIAL_SET(dock_strut, 0, strh, 0, 0, 0, 0, dock->area.x, dock->area.x + dock->area.width - 1, 0, 0, 0,
                          0);
        break;
      case OB_DIRECTION_NORTHEAST:
        switch (config_dock_orient) {
          case OB_ORIENTATION_HORZ:
            ...
            break;
          case OB_ORIENTATION_VERT:
            STRUT_PARTIAL_SET(dock_strut, 0, 0, strw, 0, 0, 0, 0, 0, dock->area.y, dock->area.y + dock->area.height - 1,
                              0, 0);
            break;
        }
        break;
      case OB_DIRECTION_WEST:
        STRUT_PARTIAL_SET(dock_strut, strw, 0, 0, 0, dock->area.y, dock->area.y + dock->area.height - 1, 0, 0, 0, 0, 0,
                          0);
        break;
      case OB_DIRECTION_EAST:
        STRUT_PARTIAL_SET(dock_strut, 0, 0, strw, 0, 0, 0, 0, 0, dock->area.y, dock->area.y + dock->area.height - 1, 0,
                          0);
        break;
      case OB_DIRECTION_SOUTHWEST:
        switch (config_dock_orient) {
          case OB_ORIENTATION_HORZ:
            STRUT_PARTIAL_SET(dock_strut, 0, 0, 0, strh, 0, 0, 0, 0, 0, 0, dock->area.x,
  ...
  if (dock->dock_apps) {
    ...
    XMoveResizeWindow(obt_display, dock->frame, dock->area.x, dock->area.y, dock->area.width, dock->area.height);
...
void dock_app_drag(ObDockApp* app, XMotionEvent* e) {
  ...
  if (!(x >= dock->area.x && y >= dock->area.y && x < dock->area.x + dock->area.width &&
...
void dock_hide(gboolean hide) {
  if (!hide) {
    if (dock->hidden && config_dock_hide) {
      show_timeout_id = g_timeout_add_full(G_PRIORITY_DEFAULT, config_dock_show_delay, show_timeout, &show_timeout_id,
                                           destroy_timeout);
    }
    ...
    if (!dock->hidden && config_dock_hide) {
      hide_timeout_id = g_timeout_add_full(G_PRIORITY_DEFAULT, config_dock_hide_delay, hide_timeout, &hide_timeout_id,
                                           destroy_timeout);
    }
