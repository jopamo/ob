...
void session_startup(gint argc, gchar** argv) {
  ...
  p = obt_paths_new();
  dir = g_build_filename(obt_paths_cache_home(p), "openbox", "sessions", NULL);
  ...
  if (ob_sm_save_file != NULL) {
    if (ob_sm_restore) {
      ob_debug_type(OB_DEBUG_SM, "Loading from session file %s", ob_sm_save_file);
    ...
    filename = g_strdup_printf("%u-%u-%u.obs", (guint)time(NULL), (guint)getpid(), g_random_int());
...
void session_shutdown(gboolean permanent) {
  ...
  if (sm_conn) {
    ...
    while (session_saved_state) {
      ...
      session_saved_state = g_list_delete_link(session_saved_state, session_saved_state);
    }
  }
}
...
static gboolean session_connect(void) {
  ...
  oldid = ob_sm_id;
  ob_debug_type(OB_DEBUG_SM, "Connecting to SM with id: %s", oldid ? oldid : "(null)");
  sm_conn = SmcOpenConnection(
      NULL, NULL, 1, 0,
      SmcSaveYourselfProcMask | SmcDieProcMask | SmcSaveCompleteProcMask | SmcShutdownCancelledProcMask, &cb, oldid,
      &ob_sm_id, SM_ERR_LEN - 1, sm_err);
  g_free(oldid);
...
static void session_setup_program(void) {
  SmPropValue vals = {.value = sm_argv[0], .length = strlen(sm_argv[0]) + 1};
  SmProp prop = {.name = g_strdup(SmProgram), .type = g_strdup(SmARRAY8), .num_vals = 1, .vals = &vals};
  SmProp* list = &prop;
...
static void session_setup_user(void) {
  ...
  SmPropValue vals = {.value = user, .length = strlen(user) + 1};
  SmProp prop = {.name = g_strdup(SmUserID), .type = g_strdup(SmARRAY8), .num_vals = 1, .vals = &vals};
  SmProp* list = &prop;
...
static void session_setup_restart_style(gboolean restart) {
  ...
  SmPropValue vals = {.value = &restart_hint, .length = 1};
  SmProp prop = {.name = g_strdup(SmRestartStyleHint), .type = g_strdup(SmCARD8), .num_vals = 1, .vals = &vals};
  SmProp* list = &prop;
...
static void session_setup_pid(void) {
  ...
  SmPropValue vals = {.value = pid, .length = strlen(pid) + 1};
  SmProp prop = {.name = g_strdup(SmProcessID), .type = g_strdup(SmARRAY8), .num_vals = 1, .vals = &vals};
  SmProp* list = &prop;
...
static void session_setup_clone_command(void) {
  ...
  SmProp prop = {.name = g_strdup(SmCloneCommand), .type = g_strdup(SmLISTofARRAY8), .num_vals = sm_argc, .vals = vals};
  SmProp* list = &prop;
...
static void session_setup_restart_command(void) {
  ...
  SmProp prop = {
      .name = g_strdup(SmRestartCommand), .type = g_strdup(SmLISTofARRAY8), .num_vals = sm_argc + 4, .vals = vals};
  SmProp* list = &prop;
...
static void sm_save_yourself_2(SmcConn conn, SmPointer data) {
  ...
  ob_debug_type(OB_DEBUG_SM, "  Saving session to file '%s'", ob_sm_save_file);
  if (savedata == NULL)
...
#ifdef DEBUG
  {
    const gchar* sname =
        (save_type == SmSaveLocal
             ? "SmSaveLocal"
             : (save_type == SmSaveGlobal ? "SmSaveGlobal" : (save_type == SmSaveBoth ? "SmSaveBoth" : "INVALID!!")));
...
static gboolean session_save_to_file(const ObSMSaveData* savedata) {
  ...
  if (!f) {
    success = FALSE;
    g_message(_("Unable to save the session to \"%s\": %s"), ob_sm_save_file, g_strerror(errno));
  }
  else {
    ...
    for (it = stacking_list; it; it = g_list_next(it)) {
      ...
      ob_debug_type(OB_DEBUG_SM, "Saving state for client %s", c->title);
    ...
    if (fflush(f)) {
      success = FALSE;
      g_message(_("Error while saving the session to \"%s\": %s"), ob_sm_save_file, g_strerror(errno));
    }
    fclose(f);
  }
...
static gboolean session_state_cmp(ObSessionState* s, ObClient* c) {
  ...
  ob_debug_type(OB_DEBUG_SM, "  client command: %s ", c->wm_command ? c->wm_command : "(null)");
  ...
  ob_debug_type(OB_DEBUG_SM, "  state command: %s ", s->command ? s->command : "(null)");

  if ((c->sm_client_id && s->id && !strcmp(c->sm_client_id, s->id)) ||
      (c->wm_command && s->command && !strcmp(c->wm_command, s->command))) {
    return (!strcmp(s->name, c->name) && !strcmp(s->class, c->class) && !strcmp(s->role, c->role) &&
...
static void session_load_file(const gchar* path) {
  ...
  if ((n = obt_xml_find_node(node->children, "desktopnames"))) {
    for (m = obt_xml_find_node(n->children, "name"); m; m = obt_xml_find_node(m->next, "name")) {
      session_desktop_names = g_slist_append(session_desktop_names, obt_xml_node_string(m));
    }
  }
  ...
  for (node = obt_xml_find_node(node->children, "window"); node != NULL;
    ...
    state->shaded = obt_xml_find_node(node->children, "shaded") != NULL;
    state->iconic = obt_xml_find_node(node->children, "iconic") != NULL;
    state->skip_pager = obt_xml_find_node(node->children, "skip_pager") != NULL;
    state->skip_taskbar = obt_xml_find_node(node->children, "skip_taskbar") != NULL;
    state->fullscreen = obt_xml_find_node(node->children, "fullscreen") != NULL;
    state->above = obt_xml_find_node(node->children, "above") != NULL;
    state->below = obt_xml_find_node(node->children, "below") != NULL;
    state->max_horz = obt_xml_find_node(node->children, "max_horz") != NULL;
    state->max_vert = obt_xml_find_node(node->children, "max_vert") != NULL;
    state->undecorated = obt_xml_find_node(node->children, "undecorated") != NULL;
    state->focused = obt_xml_find_node(node->children, "focused") != NULL;
  ...
  */
  for (it = session_saved_state; it; it = inext) {
    ...
    for (jt = g_list_next(it); jt; jt = jnext) {
      ...
      if (match && !strcmp(s1->name, s2->name) && !strcmp(s1->class, s2->class) && !strcmp(s1->role, s2->role)) {
        ob_debug_type(OB_DEBUG_SM, "removing duplicate %s", s2->name);
        session_state_free(s2);
        session_saved_state = g_list_delete_link(session_saved_state, jt);
        founddup = TRUE;
      }
    }
...
void session_request_logout(gboolean silent) {
  if (sm_conn) {
    SmcRequestSaveYourself(sm_conn, SmSaveGlobal, TRUE,                               /* logout */
                           (silent ? SmInteractStyleNone : SmInteractStyleAny), TRUE, /* if false, with GSM, it shows
                                                                                         the old logout prompt */
                           TRUE);                                                     /* global */
  }
  else
