...
gint main(gint argc, gchar** argv) {
  ...
  if (remote_control) {
    ...
    OBT_PROP_MSG(ob_screen, obt_root(ob_screen), OB_CONTROL, remote_control, 0, 0, 0, 0);
    obt_display_close();
    exit(EXIT_SUCCESS);
  }
  ...
  */
  ...
  cursors[OB_CURSOR_NORTHEAST] = load_cursor("top_right_corner", XC_top_right_corner);
  cursors[OB_CURSOR_EAST] = load_cursor("right_side", XC_right_side);
  cursors[OB_CURSOR_SOUTHEAST] = load_cursor("bottom_right_corner", XC_bottom_right_corner);
  cursors[OB_CURSOR_SOUTH] = load_cursor("bottom_side", XC_bottom_side);
  cursors[OB_CURSOR_SOUTHWEST] = load_cursor("bottom_left_corner", XC_bottom_left_corner);
  cursors[OB_CURSOR_WEST] = load_cursor("left_side", XC_left_side);
  cursors[OB_CURSOR_NORTHWEST] = load_cursor("top_left_corner", XC_top_left_corner);
  ...
  if (screen_annex()) { /* it will be ours! */
    ...
    do {
      ...
      {
        RrTheme* theme;
        if ((theme = RrThemeNew(ob_rr_inst, config_theme, TRUE, config_font_activewindow, config_font_inactivewindow,
                                config_font_menutitle, config_font_menuitem, config_font_activeosd,
        ...
        OBT_PROP_SETS(obt_root(ob_screen), OB_THEME, ob_rr_theme->name);
      }
      ...
      if (!reconfigure) {
        ...
        if (OBT_PROP_GET32(obt_root(ob_screen), NET_ACTIVE_WINDOW, WINDOW, &xid) && (w = window_find(xid)) &&
            WINDOW_IS_CLIENT(w)) {
  ...
  if (restart) {
    ...
    if (ob_sm_save_file != NULL || ob_sm_id != NULL) {
      ...
      l = argc + 1 + (ob_sm_save_file != NULL ? 2 : 0) + (ob_sm_id != NULL ? 2 : 0);
...
static void parse_env(void) {
  ...
  if (id) {
    ...
    ob_debug_type(OB_DEBUG_SM, "DESKTOP_AUTOSTART_ID %s supercedes --sm-client-id\n", ob_sm_id);
  }
}
