...
#ifdef USE_SM
static gboolean ice_handler(GIOChannel* source, GIOCondition cond, gpointer conn) {
  Bool b;
  IceProcessMessages(conn, NULL, &b);
  return TRUE; /* don't remove the event source */
}

static void ice_watch(IceConn conn, IcePointer data, Bool opening, IcePointer* watch_data) {
  static guint id = 0;
...
static void print_focusevent(XEvent* e) {
  ...
  switch (detail) {
    ...
    case NotifyNonlinearVirtual:
      ...
      break;
    case NotifyPointer:
      detailstr = "NotifyPointer";
      break;
    case NotifyPointerRoot:
      detailstr = "NotifyPointerRoot";
      break;
  ...
  g_assert(modestr);
  g_assert(detailstr);
  ob_debug_type(OB_DEBUG_FOCUS, "Focus%s 0x%x mode=%s detail=%s", (e->xfocus.type == FocusIn ? "In" : "Out"), win,
                modestr, detailstr);
}
...
static void event_process(const XEvent* ec, gpointer data) {
  ...
  if (e->type == FocusIn) {
    ...
    if (!wanted_focusevent(e, FALSE)) {
      if (waiting_for_focusin) {
        ...
        ob_debug_type(OB_DEBUG_FOCUS, "Focus went to an unmanaged window 0x%x !", e->xfocus.window);
      ...
      */
      ...
      ob_debug_type(OB_DEBUG_FOCUS, "Focus went to a window that is already gone");
    ...
    /* Look for the followup FocusIn */
      ...
      if (XGetInputFocus(obt_display, &win, &i) && XGetGeometry(obt_display, win, &root, &i, &i, &u, &u, &u, &u) &&
    ...
    if (e->xclient.message_type == OBT_PROP_ATOM(NET_REQUEST_FRAME_EXTENTS)) {
      ...
      OBT_PROP_SETA32(e->xclient.window, NET_FRAME_EXTENTS, CARDINAL, vals, 4);
    ...
    /* we are not to be held responsible if someone sends us an
       invalid request! */
    obt_display_ignore_errors(TRUE);
    XConfigureWindow(obt_display, window, e->xconfigurerequest.value_mask, &xwc);
    obt_display_ignore_errors(FALSE);
  }
#ifdef SYNC
  else if (obt_display_extension_sync && e->type == obt_display_extension_sync_basep + XSyncAlarmNotify) {
  ...
  if (e->type == ButtonPress || e->type == ButtonRelease) {
    ...
    /* If the button press was on some non-root window, or was physically
       on the root window... */
    if (window != obt_root(ob_screen) || e->xbutton.subwindow == None ||
...
static gboolean skip_property_change(XEvent* e, gpointer data) {
  ...
  if (e->type == PropertyNotify && e->xproperty.window == s.window) {
    ...
    if ((a == OBT_PROP_ATOM(NET_WM_NAME) || a == OBT_PROP_ATOM(WM_NAME) || a == OBT_PROP_ATOM(NET_WM_ICON_NAME) ||
        ...
        (b == OBT_PROP_ATOM(NET_WM_NAME) || b == OBT_PROP_ATOM(WM_NAME) || b == OBT_PROP_ATOM(NET_WM_ICON_NAME) ||
  ...
  switch (e->type) {
    ...
    case MotionNotify:
      ...
      switch (con) {
        ...
        case OB_FRAME_CONTEXT_TRCORNER:
          ...
          if (client->frame->max_hover || client->frame->desk_hover || client->frame->shade_hover ||
              client->frame->iconify_hover || client->frame->close_hover) {
            client->frame->max_hover = client->frame->desk_hover = client->frame->shade_hover =
                client->frame->iconify_hover = client->frame->close_hover = FALSE;
            frame_adjust_state(client->frame);
          }
          break;
        default:
      ...
      switch (con) {
        ...
        case OB_FRAME_CONTEXT_TRCORNER:
          ...
          if (e->xcrossing.mode == NotifyGrab) {
            client->frame->max_press = client->frame->desk_press = client->frame->shade_press =
                client->frame->iconify_press = client->frame->close_press = FALSE;
          }
          break;
    ...
    case ConfigureRequest: {
      ...
      */
      ...
      ob_debug(
          "ConfigureRequest for \"%s\" desktop %d wmstate %d "
          "visible %d",
          client->title, screen_desktop, client->wmstate, client->frame->visible);
      ob_debug("                     x %d y %d w %d h %d b %d", x, y, w, h, client->border_width);
      ...
      ob_debug(
          "ConfigureRequest x(%d) %d y(%d) %d w(%d) %d h(%d) %d "
          "move %d resize %d",
      ...
      */
...

      {
        ...
        ob_debug("Granting ConfigureRequest x %d y %d w %d h %d", x, y, w, h);
    ...
    case UnmapNotify:
      ob_debug(
          "UnmapNotify for window 0x%x eventwin 0x%x sendevent %d "
          "ignores left %d",
          client->window, e->xunmap.event, e->xunmap.from_configure, client->ignore_unmaps);
    ...
    case ClientMessage:
      ...
      if (msgtype == OBT_PROP_ATOM(WM_CHANGE_STATE)) {
        ...
        if (!more_client_message_event(client->window, msgtype) &&
            ((unsigned)e->xclient.data.l[0] < screen_num_desktops || (unsigned)e->xclient.data.l[0] == DESKTOP_ALL)) {
          client_set_desktop(client, (unsigned)e->xclient.data.l[0], FALSE, FALSE);
        }
      }
        ...
        ob_debug("net_wm_state %s %ld %ld for 0x%lx",
                 (e->xclient.data.l[0] == 0   ? "Remove"
                  ...
                  : e->xclient.data.l[0] == 2 ? "Toggle"
                                              : "INVALID"),
                 e->xclient.data.l[1], e->xclient.data.l[2], client->window);

        /* ignore enter events caused by these like ob actions do */
        if (!config_focus_under_mouse)
          ignore_start = event_start_ignore_all_enters();
        client_set_state(client, e->xclient.data.l[0], e->xclient.data.l[1], e->xclient.data.l[2]);
        if (!config_focus_under_mouse)
          event_end_ignore_all_enters(ignore_start);
      }
        ...
        */
        ...
        if ((Atom)e->xclient.data.l[2] == OBT_PROP_ATOM(NET_WM_MOVERESIZE_SIZE_TOPLEFT) ||
            (Atom)e->xclient.data.l[2] == OBT_PROP_ATOM(NET_WM_MOVERESIZE_SIZE_TOP) ||
            (Atom)e->xclient.data.l[2] == OBT_PROP_ATOM(NET_WM_MOVERESIZE_SIZE_TOPRIGHT) ||
            (Atom)e->xclient.data.l[2] == OBT_PROP_ATOM(NET_WM_MOVERESIZE_SIZE_RIGHT) ||
            (Atom)e->xclient.data.l[2] == OBT_PROP_ATOM(NET_WM_MOVERESIZE_SIZE_RIGHT) ||
            (Atom)e->xclient.data.l[2] == OBT_PROP_ATOM(NET_WM_MOVERESIZE_SIZE_BOTTOMRIGHT) ||
            (Atom)e->xclient.data.l[2] == OBT_PROP_ATOM(NET_WM_MOVERESIZE_SIZE_BOTTOM) ||
            (Atom)e->xclient.data.l[2] == OBT_PROP_ATOM(NET_WM_MOVERESIZE_SIZE_BOTTOMLEFT) ||
            (Atom)e->xclient.data.l[2] == OBT_PROP_ATOM(NET_WM_MOVERESIZE_SIZE_LEFT) ||
            (Atom)e->xclient.data.l[2] == OBT_PROP_ATOM(NET_WM_MOVERESIZE_MOVE) ||
            (Atom)e->xclient.data.l[2] == OBT_PROP_ATOM(NET_WM_MOVERESIZE_SIZE_KEYBOARD) ||
            (Atom)e->xclient.data.l[2] == OBT_PROP_ATOM(NET_WM_MOVERESIZE_MOVE_KEYBOARD)) {
          moveresize_start(client, e->xclient.data.l[0], e->xclient.data.l[1], e->xclient.data.l[3],
                           e->xclient.data.l[2]);
        }
        else if ((Atom)e->xclient.data.l[2] == OBT_PROP_ATOM(NET_WM_MOVERESIZE_CANCEL))
        ...
        ob_debug("MOVERESIZE x %d %d y %d %d (gravity %d)", e->xclient.data.l[0] & 1 << 8, x,
                 e->xclient.data.l[0] & 1 << 9, y, client->gravity);
        ...
        if (e->xclient.data.l[0] != 2) {
          ...
          if (e->xclient.data.l[2] == Below || e->xclient.data.l[2] == BottomIf || e->xclient.data.l[2] == Above ||
              e->xclient.data.l[2] == TopIf || e->xclient.data.l[2] == Opposite) {
            gulong ignore_start;

            if (!config_focus_under_mouse)
              ignore_start = event_start_ignore_all_enters();
            /* just raise, don't activate */
            stacking_restack_request(client, sibling, e->xclient.data.l[2]);
            if (!config_focus_under_mouse)
              event_end_ignore_all_enters(ignore_start);
    ...
    case PropertyNotify:
      ...
      else if (msgtype == OBT_PROP_ATOM(NET_WM_NAME) || msgtype == OBT_PROP_ATOM(WM_NAME) ||
               msgtype == OBT_PROP_ATOM(NET_WM_ICON_NAME) || msgtype == OBT_PROP_ATOM(WM_ICON_NAME)) {
        client_update_title(client);
      }
      else if (msgtype == OBT_PROP_ATOM(WM_PROTOCOLS)) {
        client_update_protocols(client);
      }
      else if (msgtype == OBT_PROP_ATOM(NET_WM_STRUT) || msgtype == OBT_PROP_ATOM(NET_WM_STRUT_PARTIAL)) {
...
static gboolean event_handle_menu_input(XEvent* ev) {
  ...
  if (ev->type == ButtonRelease || ev->type == ButtonPress) {
    ...
    if ((ev->xbutton.button < 4 || ev->xbutton.button > 5) &&
        ((ev->type == ButtonRelease && menu_hide_delay_reached()) || ev->type == ButtonPress)) {
      if ((e = menu_entry_frame_under(ev->xbutton.x_root, ev->xbutton.y_root))) {
    ...
    /* Allow control while going thru the menu */
      ...
      if (sym == XK_Escape) {
        ...
        if (frame->parent) {
          ...
          menu_frame_select(frame->parent, frame->parent->selected, TRUE);
        }
        ret = TRUE;
      }
      ...
      else if (frame->entries && (unikey = obt_keyboard_keypress_to_unichar(menu_frame_ic(frame), ev))) {
        GList* start;
        GList* it;
    ...
    /* Use KeyRelease events for running things so that the key release
      ...
      if (frame->press_keycode == ev->xkey.keycode && frame->got_press && frame->press_doexec) {
        if (frame->selected)
...
static gboolean event_look_for_menu_enter(XEvent* ev, gpointer data) {
  ...
  ObMenuEntryFrame* e;
  return ev->type == EnterNotify && (e = g_hash_table_lookup(menu_frame_map, &ev->xcrossing.window)) && e->frame == f &&
         !e->ignore_enters;
}
  ...
  switch (ev->type) {
    case MotionNotify:
      ...
      if ((e = g_hash_table_lookup(menu_frame_map, &ev->xmotion.window))) {
        if (e->ignore_enters)
          --e->ignore_enters;
        else if (!(f = find_active_menu()) || f == e->frame || f->parent == e->frame || f->child == e->frame)
...
static void event_ignore_enter_range(gulong start, gulong end) {
  ...
  ob_debug_type(OB_DEBUG_FOCUS, "ignoring enters from %lu until %lu", r->start, r->end);
...
static Time next_time(void) {
  ...
  /* Generate a timestamp so there is guaranteed at least one in the queue
     eventually */
  XChangeProperty(obt_display, screen_support_win, OBT_PROP_ATOM(WM_CLASS), OBT_PROP_ATOM(STRING), 8, PropModeAppend,
                  NULL, 0);
