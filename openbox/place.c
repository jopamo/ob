...
gint cmp_foreground(const void* a, const void* b) {
  ...
  if (h1->flags & HEAD_PERAPP)
    --i;
  if (h2->flags & HEAD_PERAPP)
    ++i;
  if (i)
    return i;
...
static Rect* choose_monitor(ObClient* c, gboolean client_to_be_foregrounded, ObAppSettings* settings) {
  Rect* area;
  ObPlaceHead* choice;
  guint i;
  ObClient* p;
  ...
  if (i < screen_num_monitors) {
    ...
    if (settings && settings->monitor_type == OB_PLACE_MONITOR_PRIMARY)
  ...
  if (i < screen_num_monitors) {
    if (config_place_monitor == OB_PLACE_MONITOR_ACTIVE)
      choice[i].flags |= HEAD_PLACED;
    if (settings && settings->monitor_type == OB_PLACE_MONITOR_ACTIVE)
      choice[i].flags |= HEAD_PERAPP;
  }
  ...
  if (i < screen_num_monitors) {
    if (config_place_monitor == OB_PLACE_MONITOR_MOUSE)
      choice[i].flags |= HEAD_PLACED;
    if (settings && settings->monitor_type == OB_PLACE_MONITOR_MOUSE)
      choice[i].flags |= HEAD_PERAPP;
  }
  ...
  qsort(choice, screen_num_monitors, sizeof(ObPlaceHead), client_to_be_foregrounded ? cmp_foreground : cmp_background);
...
static gboolean place_under_mouse(ObClient* client, gint* x, gint* y, Size frame_size) {
  gint l, r, t, b;
  gint px, py;
  Rect* area;
...
static gboolean place_per_app_setting_position(ObClient* client,
                                               Rect* screen,
                                               gint* x,
                                               gint* y,
  ...
  screen_apply_gravity_point(x, y, frame_size.width, frame_size.height, &settings->position, screen);

  return TRUE;
}

static void place_per_app_setting_size(ObClient* client, Rect* screen, gint* w, gint* h, ObAppSettings* settings) {
  if (!settings)
    return;
...
static gboolean place_transient_splash(ObClient* client, Rect* area, gint* x, gint* y, Size frame_size) {
  ...
  if (client->type == OB_CLIENT_TYPE_DIALOG || client->type == OB_CLIENT_TYPE_SPLASH) {
...
static gboolean place_least_overlap(ObClient* c, Rect* head, int* x, int* y, Size frame_size) {
  ...
  if (!ignore_windows) {
    ...
    for (it = client_list; it != NULL; it = g_list_next(it)) {
      ...
      if (c->desktop != DESKTOP_ALL) {
        if (maybe_client->desktop != c->desktop && maybe_client->desktop != DESKTOP_ALL)
          continue;
      }
      else {
        if (maybe_client->desktop != screen_desktop && maybe_client->desktop != DESKTOP_ALL)
          continue;
      }

      potential_overlap_clients = g_slist_prepend(potential_overlap_clients, maybe_client);
      n_client_rects += 1;
    }
  }
  ...
  if (n_client_rects) {
    ...
    place_overlap_find_least_placement(client_rects, n_client_rects, head, &frame_size, &result);
    *x = result.x;
    *y = result.y;
  }
...
static gboolean should_set_client_position(ObClient* client, ObAppSettings* settings) {
...
gboolean place_client(ObClient* client,
