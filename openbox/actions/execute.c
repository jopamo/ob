...
static void free_func(gpointer options) {
  ...
  if (o) {
    ...
    g_free(o->prompt);
    if (o->data)
      g_slice_free(ObActionsData, o->data);
...
static gchar* expand_variables(gchar* cmd, ObActionsData* data) {
  ...
  while ((c = strchr(before, '$'))) {
    if ((c[1] == 'p' || c[1] == 'P') && (c[2] == 'i' || c[2] == 'I') && (c[3] == 'd' || c[3] == 'D') &&
      ...
      *c = '\0';
      tmp = expand;
      expand = g_strdup_printf("%s%s%u", (expand ? expand : ""), before, data->client ? data->client->pid : 0);
      g_free(tmp);
      ...
      *c = '\0';
      tmp = expand;
      expand = g_strdup_printf("%s%s%lu", (expand ? expand : ""), before, data->client ? data->client->window : 0);
      g_free(tmp);

      before = c + 4; /* 4 = strlen("$wid") */
    }
    else if ((c[1] == 'p' || c[1] == 'P') && (c[2] == 'o' || c[2] == 'O') && (c[3] == 'i' || c[3] == 'I') &&
             (c[4] == 'n' || c[4] == 'N') && (c[5] == 't' || c[5] == 'T') && (c[6] == 'e' || c[6] == 'E') &&
             (c[7] == 'r' || c[7] == 'R') && !g_ascii_isalnum(c[8])) {
      /* found $pointer */
      gchar* tmp;

      *c = '\0';
      tmp = expand;
      expand = g_strdup_printf("%s%s%u %u", (expand ? expand : ""), before, data->x, data->y);
      g_free(tmp);
      ...
      *c = '\0';
      tmp = expand;
      expand = g_strdup_printf("%s%s$", (expand ? expand : ""), before);
      g_free(tmp);
...
static gboolean run_func(ObActionsData* data, gpointer options) {
  ...
  if (o->prompt) {
    ObPrompt* p;
    Options* ocp;
    ObPromptAnswer answers[] = {{_("No"), 0}, {_("Yes"), 1}};

    ocp = dup_options(options, data);
    p = prompt_new(o->prompt, _("Execute"), answers, 2, 0, 0, prompt_cb, prompt_cleanup, ocp);
  ...
  if (!g_shell_parse_argv(cmd, NULL, &argv, &e)) {
    ...
    if (o->sn) {
      ...
      sn_setup_spawn_environment(program, o->sn_name, o->sn_icon, o->sn_wmclass,
