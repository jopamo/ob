...
static gpointer setup_grow_func(GHashTable* options);
static gpointer setup_fill_func(GHashTable* options);
static gpointer setup_shrink_func(GHashTable* options);
...
void action_growtoedge_startup(void) {
  actions_register_opt("GrowToEdge", setup_grow_func, free_func, run_func);
  actions_register_opt("GrowToFill", setup_fill_func, free_func, run_func);
  actions_register_opt("ShrinkToEdge", setup_shrink_func, free_func, run_func);
  /* 3.4-compatibility */
  actions_register_opt("GrowToEdgeNorth", setup_north_func, free_func, run_func);
  actions_register_opt("GrowToEdgeSouth", setup_south_func, free_func, run_func);
  actions_register_opt("GrowToEdgeEast", setup_east_func, free_func, run_func);
  actions_register_opt("GrowToEdgeWest", setup_west_func, free_func, run_func);
}
...
static gboolean do_grow_all_edges(ObActionsData* data, ObClientDirectionalResizeType resize_type) {
  gint x, y, w, h;
  gint temp_x, temp_y, temp_w, temp_h;

  client_find_resize_directional(data->client, OB_DIRECTION_NORTH, resize_type, &temp_x, &temp_y, &temp_w, &temp_h);
  y = temp_y;
  h = temp_h;

  client_find_resize_directional(data->client, OB_DIRECTION_SOUTH, resize_type, &temp_x, &temp_y, &temp_w, &temp_h);
  h += temp_h - data->client->area.height;

  client_find_resize_directional(data->client, OB_DIRECTION_WEST, resize_type, &temp_x, &temp_y, &temp_w, &temp_h);
  x = temp_x;
  w = temp_w;

  client_find_resize_directional(data->client, OB_DIRECTION_EAST, resize_type, &temp_x, &temp_y, &temp_w, &temp_h);
  ...
  if (x == data->client->area.x && y == data->client->area.y && w == data->client->area.width &&
...
static gboolean run_func(ObActionsData* data, gpointer options) {
  ...
  gboolean doing_vertical_resize = o->dir == OB_DIRECTION_NORTH || o->dir == OB_DIRECTION_SOUTH || o->fill;
  ...
  if (!o->shrink) {
    ...
    /* Try grow. */
    client_find_resize_directional(data->client, o->dir, CLIENT_RESIZE_GROW, &x, &y, &w, &h);
  ...
  ObDirection opposite =
      (o->dir == OB_DIRECTION_NORTH
           ...
           : (o->dir == OB_DIRECTION_SOUTH ? OB_DIRECTION_NORTH
                                           : (o->dir == OB_DIRECTION_EAST ? OB_DIRECTION_WEST : OB_DIRECTION_EAST)));

  gint x, y, w, h;
  gint half;

  client_find_resize_directional(data->client, opposite, CLIENT_RESIZE_SHRINK, &x, &y, &w, &h);
