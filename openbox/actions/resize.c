...
static gboolean run_func(ObActionsData* data, gpointer options) {
  ...
  if (data->client) {
    ...
    if (!data->button)
      ...
      corner = pick_corner(data->x, data->y, c->frame->area.x, c->frame->area.y,
                           ...
                           c->area.height + c->frame->size.top + c->frame->size.bottom, c->shaded);
...
static guint32 pick_corner(gint x, gint y, gint cx, gint cy, gint cw, gint ch, gboolean shaded) {
  ...
  if (cx + cw > full->x + full->width)
    cw = full->x + full->width - cx;
  if (cy + ch > full->y + full->height)
    ch = full->y + full->height - cy;
