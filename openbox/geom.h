...
#define RECT_EQUAL_DIMS(r, x, y, w, h) ((r).x == (x) && (r).y == (y) && (r).width == (w) && (r).height == (h))
...
#define RECT_INTERSECTS_RECT(r, o) \
  ((o).x < (r).x + (r).width && (o).x + (o).width > (r).x && (o).y < (r).y + (r).height && (o).y + (o).height > (r).y)
...
#define STRUT_PARTIAL_SET(s, l, t, r, b, ls, le, ts, te, rs, re, bs, be)                                           \
  (s).left = (l), (s).top = (t), (s).right = (r), (s).bottom = (b), (s).left_start = (ls), (s).left_end = (le),    \
  (s).top_start = (ts), (s).top_end = (te), (s).right_start = (rs), (s).right_end = (re), (s).bottom_start = (bs), \
  (s).bottom_end = (be)

#define STRUT_ADD(s1, s2)                                                                                              \
  (s1).left = MAX((s1).left, (s2).left), (s1).right = MAX((s1).right, (s2).right), (s1).top = MAX((s1).top, (s2).top), \
  (s1).bottom = MAX((s1).bottom, (s2).bottom)

#define STRUT_EXISTS(s1) ((s1).left || (s1).top || (s1).right || (s1).bottom)

#define STRUT_EQUAL(s1, s2) \
  ((s1).left == (s2).left && (s1).top == (s2).top && (s1).right == (s2).right && (s1).bottom == (s2).bottom)

#define PARTIAL_STRUT_EQUAL(s1, s2)                                                                            \
  ((s1).left == (s2).left && (s1).top == (s2).top && (s1).right == (s2).right && (s1).bottom == (s2).bottom && \
   (s1).left_start == (s2).left_start && (s1).left_end == (s2).left_end && (s1).top_start == (s2).top_start && \
   (s1).top_end == (s2).top_end && (s1).right_start == (s2).right_start && (s1).right_end == (s2).right_end && \
