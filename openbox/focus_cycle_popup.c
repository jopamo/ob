...
static gboolean popup_setup(ObFocusCyclePopup* p, gboolean create_targets, gboolean refresh_targets, gboolean linear);
static void popup_render(ObFocusCyclePopup* p, const ObClient* c);

static Window create_window(Window parent, guint bwidth, gulong mask, XSetWindowAttributes* attr) {
  return XCreateWindow(obt_display, parent, 0, 0, 1, 1, bwidth, RrDepth(ob_rr_inst), InputOutput, RrVisual(ob_rr_inst),
                       mask, attr);
}
...
void focus_cycle_popup_startup(gboolean reconfig) {
  ...
  popup.a_arrow->texture[0].data.mask.color = ob_rr_theme->osd_text_active_color;

  attrib.override_redirect = True;
  attrib.border_pixel = RrColorPixel(ob_rr_theme->osd_border_color);
  popup.bg = create_window(obt_root(ob_screen), ob_rr_theme->obwidth, CWOverrideRedirect | CWBorderPixel, &attrib);
  ...
  /* create the hilite under the target icon */
  {
    ...
    color = ((guint)(tc->r & 0xff) << RrDefaultRedOffset) + ((guint)(tc->g & 0xff) << RrDefaultGreenOffset) +
            ((guint)(tc->b & 0xff) << RrDefaultBlueOffset);
    ...
    for (x = 0; x < HILITE_SIZE; x++)
      for (y = 0; y < HILITE_SIZE; y++) {
        ...
        if (x < HILITE_WIDTH || x >= HILITE_SIZE - HILITE_WIDTH || y < HILITE_WIDTH ||
        ...
        p[o++] = color + ((guint)a << RrDefaultAlphaOffset);
      }
  }
...
static gboolean popup_setup(ObFocusCyclePopup* p, gboolean create_targets, gboolean refresh_targets, gboolean linear) {
  ...
  maxwidth = 0;
  n = 0;
  for (it = g_list_last(linear ? client_list : focus_order); it; it = g_list_previous(it)) {
    ObClient* ft = it->data;
    ...
    if (focus_cycle_valid(ft)) {
      ...
      if (!rit) {
        ...
        if (!create_targets) {
          ...
          ObFocusCyclePopupTarget* t = g_slice_new(ObFocusCyclePopupTarget);
...
static void popup_render(ObFocusCyclePopup* p, const ObClient* c) {
  ...
  if (mode == OB_FOCUS_CYCLE_POPUP_MODE_ICONS) {
    ...
    icon_rows = MIN(p->n_targets, (4 * screen_area->height / 5)   /* 80% of the screen */
  ...
  if (p->scroll < p->n_targets - icon_rows && mode == OB_FOCUS_CYCLE_POPUP_MODE_LIST) {
  ...
  if (showing_arrows)
    h += ob_rr_theme->up_arrow_mask->height + OUTSIDE_BORDER + ob_rr_theme->down_arrow_mask->height + OUTSIDE_BORDER;
  ...
  x = screen_area->x + (screen_area->width - (w + ob_rr_theme->obwidth * 2)) / 2;
  y = screen_area->y + (screen_area->height - (h + ob_rr_theme->obwidth * 2)) / 2;
  ...
  if (!p->mapped) {
    ...
    if (mode == OB_FOCUS_CYCLE_POPUP_MODE_ICONS) {
      /* position the text */
      XMoveResizeWindow(obt_display, p->icon_mode_text, icon_mode_textx, icon_mode_texty, textw, texth);
      ...
      /* position the arrows */
      XMoveResizeWindow(obt_display, p->list_mode_up, up_arrow_x, up_arrow_y, ob_rr_theme->up_arrow_mask->width,
                        ob_rr_theme->up_arrow_mask->height);
      XMoveResizeWindow(obt_display, p->list_mode_down, down_arrow_x, down_arrow_y, ob_rr_theme->down_arrow_mask->width,
  ...
  if (!p->mapped && mode == OB_FOCUS_CYCLE_POPUP_MODE_LIST) {
    p->a_arrow->texture[0].data.mask.mask = ob_rr_theme->up_arrow_mask;
    p->a_arrow->surface.parent = p->a_bg;
    p->a_arrow->surface.parentx = up_arrow_x;
    p->a_arrow->surface.parenty = up_arrow_y;
    RrPaint(p->a_arrow, p->list_mode_up, ob_rr_theme->up_arrow_mask->width, ob_rr_theme->up_arrow_mask->height);

    p->a_arrow->texture[0].data.mask.mask = ob_rr_theme->down_arrow_mask;
    p->a_arrow->surface.parent = p->a_bg;
    p->a_arrow->surface.parentx = down_arrow_x;
    p->a_arrow->surface.parenty = down_arrow_y;
    RrPaint(p->a_arrow, p->list_mode_down, ob_rr_theme->down_arrow_mask->width, ob_rr_theme->down_arrow_mask->height);
  }
  ...
  for (i = 0, it = p->targets; it; ++i, it = g_list_next(it)) {
    ...
    if (!p->mapped || newtarget == target || p->last_target == target || last_scroll != p->scroll) {
      ...
      icony = t + (showing_arrows ? ob_rr_theme->up_arrow_mask->height + OUTSIDE_BORDER : 0) +
              (row * MAX(texth, HILITE_SIZE)) + MAX(texth - HILITE_SIZE, 0) / 2;
      ...
      /* position the icon */
      XMoveResizeWindow(obt_display, target->iconwin, iconx, icony, HILITE_SIZE, HILITE_SIZE);

      /* position the text */
      if (mode == OB_FOCUS_CYCLE_POPUP_MODE_LIST)
        XMoveResizeWindow(obt_display, target->textwin, list_mode_textx, list_mode_texty, textw, texth);
      ...
      p->a_icon->texture[0].data.image.alpha = target->client->iconic ? OB_ICONIC_ALPHA : 0xff;
      p->a_icon->texture[0].data.image.image = target->icon;

      /* Draw the hilite? */
      p->a_icon->texture[1].type = (target == newtarget) ? RR_TEXTURE_RGBA : RR_TEXTURE_NONE;
      ...
      if (mode == OB_FOCUS_CYCLE_POPUP_MODE_LIST || target == newtarget) {
        ...
        text->surface.parentx = mode == OB_FOCUS_CYCLE_POPUP_MODE_ICONS ? icon_mode_textx : list_mode_textx;
        text->surface.parenty = mode == OB_FOCUS_CYCLE_POPUP_MODE_ICONS ? icon_mode_texty : list_mode_texty;
        RrPaint(text, (mode == OB_FOCUS_CYCLE_POPUP_MODE_ICONS ? p->icon_mode_text : target->textwin), textw, texth);
      }
    }
  }
...
void focus_cycle_popup_show(ObClient* c, ObFocusCyclePopupMode mode, gboolean linear) {
  g_assert(c != NULL);
...
void focus_cycle_popup_single_show(struct _ObClient* c) {
  ...
  if (!single_popup->popup->mapped) {
    ...
    icon_popup_position(single_popup, CenterGravity, a->x + a->width / 2, a->y + a->height / 2);
...
ObClient* focus_cycle_popup_refresh(ObClient* target, gboolean redraw, gboolean linear) {
  if (!popup.mapped)
    return NULL;
