...
static ObClient* focus_fallback_target(gboolean allow_refocus,
  ...
  if (allow_pointer && config_focus_follow)
    if ((c = client_under_pointer()) && (allow_refocus || client_focus_target(c) != old) &&
        (client_normal(c) && client_focus(c))) {
  ...
  for (it = focus_order; it; it = g_list_next(it)) {
    ...
    */
    if ((allow_omnipresent || c->desktop == screen_desktop) &&
        focus_valid_target(c, screen_desktop, TRUE, FALSE, FALSE, TRUE, FALSE, FALSE, FALSE) && !c->shaded &&
  ...
  for (it = focus_order; it; it = g_list_next(it)) {
    ...
    */
    if (focus_valid_target(c, screen_desktop, TRUE, FALSE, FALSE, TRUE, FALSE, TRUE, FALSE) &&
        (allow_refocus || client_focus_target(c) != old) && client_focus(c)) {
...
void focus_nothing(void) {
  ...
  XSetInputFocus(obt_display, screen_support_win, RevertToPointerRoot, event_time());
}
...
void focus_order_to_top(ObClient* c) {
  ...
  if (!c->iconic) {
    ...
    for (it = focus_order; it && !((ObClient*)it->data)->iconic; it = g_list_next(it))
      ;
...
/*! Returns if a focus target has valid group siblings that can be cycled
  to in its place */
static gboolean focus_target_has_siblings(ObClient* ft, gboolean iconic_windows, gboolean all_desktops)

{
  ...
  for (it = ft->group->members; it; it = g_slist_next(it)) {
    ...
    if (c != ft && c->type == OB_CLIENT_TYPE_NORMAL &&
        focus_valid_target(c, screen_desktop, TRUE, iconic_windows, all_desktops, TRUE, FALSE, FALSE, FALSE)) {
      return TRUE;
    }
  }
  return FALSE;
...
gboolean focus_valid_target(ObClient* ft,
  ...
  if (dock_windows || desktop_windows)
    ok = ok &&
         ((dock_windows && ft->type == OB_CLIENT_TYPE_DOCK) || (desktop_windows && ft->type == OB_CLIENT_TYPE_DESKTOP));
  ...
  ok = ok && (!ft->skip_taskbar || (ft->modal || user_request || (ft->iconic && !ft->parents) ||
                                    ft->demands_attention || ft->type == OB_CLIENT_TYPE_DIALOG));
     ...
     you choose this window and bring the modal one here */
  {
    ...
    ok = ok && (ft == cft || !focus_valid_target(cft, screen_desktop, TRUE, iconic_windows, all_desktops,
                                                 nonhilite_windows, dock_windows, desktop_windows, FALSE));
  }
