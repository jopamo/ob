...
ObActionsDefinition* do_register(const gchar* name, ObActionsDataFreeFunc free, ObActionsRunFunc run) {
  GSList* it;
  ObActionsDefinition* def;
...
gboolean actions_set_can_stop(const gchar* name, gboolean can_stop) {
  GSList* it;
  ObActionsDefinition* def;
...
gboolean actions_interactive_input_event(XEvent* e) {
  gboolean used = FALSE;
  if (interactive_act) {
    if (!interactive_act->i_input(interactive_initial_state, e, grab_input_context(), interactive_act->options,
                                  &used)) {
