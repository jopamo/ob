...
void stacking_set_list(void) {
  ...
  */
  ...
  OBT_PROP_SETA32(obt_root(ob_screen), NET_CLIENT_LIST_STACKING, WINDOW, (gulong*)windows, i);
    ...
    for (it = stacking_list; !ret && it; it = g_list_next(it)) {
      if (WINDOW_IS_CLIENT(it->data)) {
        ...
        /* only look at windows in the same layer and that are
           visible */
        if (c->layer == client->layer && !c->iconic &&
            (c->desktop == client->desktop || c->desktop == DESKTOP_ALL || client->desktop == DESKTOP_ALL)) {
          GSList* sit;
...
static gboolean stacking_occluded(ObClient* client, ObWindow* sibling_win) {
  ...
  for (it = g_list_previous(g_list_find(stacking_list, client)); it; it = g_list_previous(it))
    if (WINDOW_IS_CLIENT(it->data)) {
      ObClient* c = it->data;
      if (!c->iconic &&
          (c->desktop == DESKTOP_ALL || client->desktop == DESKTOP_ALL || c->desktop == client->desktop) &&
...
gboolean stacking_restack_request(ObClient* client, ObWindow* sibling_win, gint detail) {
  gboolean ret = FALSE;
  ...
  if (sibling &&
      ((client->desktop != sibling->desktop && client->desktop != DESKTOP_ALL && sibling->desktop != DESKTOP_ALL) ||
       sibling->iconic)) {
    ob_debug(
        "Setting restack sibling to NULL, they are not on the same "
  ...
  switch (detail) {
    case Below:
      ob_debug("Restack request Below for client %s sibling %s", client->title, sibling ? sibling->title : "(all)");
      /* just lower it */
      stacking_lower(CLIENT_AS_WINDOW(client));
      ret = TRUE;
      break;
    case BottomIf:
      ob_debug("Restack request BottomIf for client %s sibling %s", client->title, sibling ? sibling->title : "(all)");
      ...
      break;
    case Above:
      ob_debug("Restack request Above for client %s sibling %s", client->title, sibling ? sibling->title : "(all)");
      stacking_raise(CLIENT_AS_WINDOW(client));
      ret = TRUE;
      break;
    case TopIf:
      ob_debug("Restack request TopIf for client %s sibling %s", client->title, sibling ? sibling->title : "(all)");
      ...
      break;
    case Opposite:
      ob_debug("Restack request Opposite for client %s sibling %s", client->title, sibling ? sibling->title : "(all)");
      if (stacking_occluded(client, sibling_win)) {
        stacking_raise(CLIENT_AS_WINDOW(client));
        ret = TRUE;
      }
