...
void mouse_grab_for_client(ObClient* client, gboolean grab) {
  ...
  for (i = 0; i < OB_FRAME_NUM_CONTEXTS; ++i)
    for (it = bound_contexts[i]; it; it = g_slist_next(it)) {
      ...
      if (grab)
        grab_button_full(b->button, b->state, win, mask, mode, OB_CURSOR_NONE);
      else
...
static gboolean
fire_binding(ObMouseAction a, ObFrameContext context, ObClient* c, guint state, guint button, gint x, gint y) {
  GSList* it;
  ObMouseBinding* b;
  ...
  switch (e->type) {
    case ButtonPress:
      ...
      */
      ...
      used = fire_binding(OB_MOUSE_ACTION_PRESS, context, client, e->xbutton.state, e->xbutton.button,
    ...
    case ButtonRelease:
      ...
      if (e->xbutton.button == button) {
        ...
        junk1 = XGetGeometry(obt_display, e->xbutton.window, &wjunk, &junk1, &junk2, &w, &h, &b, &ujunk);
        ...
        if (junk1) {
          if (e->xbutton.x >= (signed)-b && e->xbutton.y >= (signed)-b && e->xbutton.x < (signed)(w + b) &&
            ...
            if (lbutton == button && lwindow == e->xbutton.window &&
                e->xbutton.time - config_mouse_dclicktime <= ltime && ABS(e->xbutton.x - lx) < 8 &&
      ...
      used = fire_binding(OB_MOUSE_ACTION_RELEASE, context, client, e->xbutton.state, e->xbutton.button,
                          e->xbutton.x_root, e->xbutton.y_root) ||
             used;
      if (click)
        used = fire_binding(OB_MOUSE_ACTION_CLICK, context, client, e->xbutton.state, e->xbutton.button,
                            e->xbutton.x_root, e->xbutton.y_root) ||
               used;
      if (dclick)
        used = fire_binding(OB_MOUSE_ACTION_DOUBLE_CLICK, context, client, e->xbutton.state, e->xbutton.button,
                            e->xbutton.x_root, e->xbutton.y_root) ||
               used;
      break;
    ...
    case MotionNotify:
      if (button) {
        ...
        if (ABS(e->xmotion.x_root - px) >= config_mouse_threshold ||
          ...
          if (context == OB_FRAME_CONTEXT_MAXIMIZE || context == OB_FRAME_CONTEXT_ALLDESKTOPS ||
              context == OB_FRAME_CONTEXT_SHADE || context == OB_FRAME_CONTEXT_ICONIFY ||
              context == OB_FRAME_CONTEXT_ICON || context == OB_FRAME_CONTEXT_CLOSE)
            break;

          used = fire_binding(OB_MOUSE_ACTION_MOTION, context, client, state, button, px, py);
          button = 0;
          state = 0;
        }
      }
...
gboolean mouse_bind(const gchar* buttonstr, ObFrameContext context, ObMouseAction mact, ObActionsAct* action) {
