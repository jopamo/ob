...
void obt_xml_register(ObtXmlInst* i, const gchar* tag, ObtXmlCallback func, gpointer data) {
  struct Callback* c;
    ...
    if (stat(path, &s) >= 0) {
      ...
      if (i->doc) {
        ...
        if (!i->root) {
          ...
          i->root = NULL;
          g_message(
              "XML document %s is of wrong type. Root "
              "node is not '%s'",
              path, root_node);
        }
        else {
...
gboolean obt_xml_load_file(ObtXmlInst* i, const gchar* path, const gchar* root_node) {
  GSList* paths;
  gboolean r;
...
gboolean obt_xml_load_config_file(ObtXmlInst* i, const gchar* domain, const gchar* filename, const gchar* root_node) {
...
gboolean obt_xml_load_theme_file(ObtXmlInst* i,
  ...
  paths = g_slist_append(paths, g_build_filename(g_get_home_dir(), ".themes", theme, NULL));

  for (it = obt_paths_data_dirs(i->xdg_paths); it; it = g_slist_next(it))
    paths = g_slist_append(paths, g_build_filename(it->data, "themes", theme, NULL));
...
gboolean obt_xml_load_mem(ObtXmlInst* i, gpointer data, guint len, const gchar* root_node) {
  gboolean r = FALSE;
  ...
  if (i->doc) {
    ...
    if (!i->root) {
      ...
      i->root = NULL;
      g_message(
          "XML Document in given memory is of wrong "
...
gboolean obt_xml_last_error(ObtXmlInst* inst) {
  return inst->last_error_file && inst->last_error_line >= 0 && inst->last_error_message;
}
...
gboolean obt_xml_attr_bool(xmlNodePtr node, const gchar* name, gboolean* value) {
...
gboolean obt_xml_attr_contains(xmlNodePtr node, const gchar* name, const gchar* val) {
  xmlChar* c = xmlGetProp(node, (const xmlChar*)name);
  gboolean r = FALSE;
  if (c) {
    g_strstrip((char*)c); /* strip leading/trailing whitespace */
