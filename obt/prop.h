...
typedef enum {
  ...
  OBT_PROP_NET_WM_BYPASS_COMPOSITOR,
  ...
  OBT_PROP_NET_WM_WINDOW_TYPE_TOOLTIP,
  OBT_PROP_NET_WM_WINDOW_TYPE_NOTIFICATION,
  OBT_PROP_NET_WM_WINDOW_TYPE_COMBO,
  OBT_PROP_NET_WM_WINDOW_TYPE_DND,
...
gboolean obt_prop_get_text(Window win, Atom prop, ObtPropTextType type, gchar** ret);
gboolean obt_prop_get_array_text(Window win, Atom prop, ObtPropTextType type, gchar*** ret);
...
#define OBT_PROP_GET32(win, prop, type, ret) (obt_prop_get32(win, OBT_PROP_ATOM(prop), OBT_PROP_ATOM(type), ret))
#define OBT_PROP_GETA32(win, prop, type, ret, nret) \
  (obt_prop_get_array32(win, OBT_PROP_ATOM(prop), OBT_PROP_ATOM(type), ret, nret))
#define OBT_PROP_GETS(win, prop, ret) (obt_prop_get_text(win, OBT_PROP_ATOM(prop), 0, ret))
#define OBT_PROP_GETSS(win, prop, ret) (obt_prop_get_array_text(win, OBT_PROP_ATOM(prop), 0, ret))
...
#define OBT_PROP_GETSS_TYPE(win, prop, type, ret) \
  (obt_prop_get_array_text(win, OBT_PROP_ATOM(prop), OBT_PROP_TEXT_##type, ret))

#define OBT_PROP_GETS_UTF8(win, prop, ret) OBT_PROP_GETS_TYPE(win, prop, UTF8_STRING, ret)
#define OBT_PROP_GETSS_UTF8(win, prop, ret) OBT_PROP_GETSS_TYPE(win, prop, UTF8_STRING, ret)
#define OBT_PROP_GETS_XPCS(win, prop, ret) OBT_PROP_GETS_TYPE(win, prop, STRING_XPCS, ret)

#define OBT_PROP_SET32(win, prop, type, val) (obt_prop_set32(win, OBT_PROP_ATOM(prop), OBT_PROP_ATOM(type), val))
#define OBT_PROP_SETA32(win, prop, type, val, num) \
  (obt_prop_set_array32(win, OBT_PROP_ATOM(prop), OBT_PROP_ATOM(type), val, num))
#define OBT_PROP_SETS(win, prop, val) (obt_prop_set_text(win, OBT_PROP_ATOM(prop), val))
#define OBT_PROP_SETSS(win, prop, strs) (obt_prop_set_array_text(win, OBT_PROP_ATOM(prop), strs))

#define OBT_PROP_ERASE(win, prop) (obt_prop_erase(win, OBT_PROP_ATOM(prop)))

#define OBT_PROP_MSG(screen, about, msgtype, data0, data1, data2, data3, data4)               \
  (obt_prop_message(screen, about, OBT_PROP_ATOM(msgtype), data0, data1, data2, data3, data4, \
                    SubstructureNotifyMask | SubstructureRedirectMask))

#define OBT_PROP_MSG_TO(to, about, msgtype, data0, data1, data2, data3, data4, mask) \
  (obt_prop_message_to(to, about, OBT_PROP_ATOM(msgtype), data0, data1, data2, data3, data4, mask))
